shadow$provide.module$node_modules$codemirror$lib$codemirror = function(global, require, module, exports) {
  (function(global, factory) {
    ...
    function elt(tag, content, className, style) {
      ...
      style && (tag.style.cssText = style);
      if ("string" == typeof content) {
        tag.appendChild(document.createTextNode(content));
      } else if (content) {
        for (className = 0; className < content.length; ++className) {
          tag.appendChild(content[className]);
        }
      }
      return tag;
    }
    ...
    function countColumn(string, end, tabSize, startIndex, startValue) {
      ...
      for (startValue = startValue || 0;;) {
        var nextTab = string.indexOf("\t", startIndex);
        if (0 > nextTab || nextTab >= end) {
          return startValue + (end - startIndex);
        }
        startValue += nextTab - startIndex;
        startValue += tabSize - startValue % tabSize;
    ...
    function findFirst(pred, from, to) {
      for (var dir = from > to ? -1 : 1;;) {
        ...
        var midF = (from + to) / 2;
        midF = 0 > dir ? Math.ceil(midF) : Math.floor(midF);
        if (midF == from) {
          return pred(midF) ? from : to;
        }
    ...
    function off(emitter, type, f) {
      if (emitter.removeEventListener) {
        ...
        var arr = (emitter = emitter._handlers) && emitter[type];
        arr && (f = indexOf(arr, f), -1 < f && (emitter[type] = arr.slice(0, f).concat(arr.slice(f + 1))));
      }
    }
    ...
    function signalCursorActivity(cm) {
      ...
      if (arr) {
        cm = cm.curOp.cursorActivityHandlers || (cm.curOp.cursorActivityHandlers = []);
        for (var i = 0; i < arr.length; ++i) {
          -1 == indexOf(cm, arr[i]) && cm.push(arr[i]);
        }
      }
    }
    ...
    function getMode(options, spec) {
      ...
      options = mfactory(options, spec);
      if (modeExtensions.hasOwnProperty(spec.name)) {
        mfactory = modeExtensions[spec.name];
        for (var prop in mfactory) {
          mfactory.hasOwnProperty(prop) && (options.hasOwnProperty(prop) && (options["_" + prop] = options[prop]), options[prop] = mfactory[prop]);
        }
      }
      options.name = spec.name;
      spec.helperType && (options.helperType = spec.helperType);
      if (spec.modeProps) {
        for (var prop$1 in spec.modeProps) {
          options[prop$1] = spec.modeProps[prop$1];
        }
      }
      return options;
    }
    function extendMode(mode, properties) {
      mode = modeExtensions.hasOwnProperty(mode) ? modeExtensions[mode] : modeExtensions[mode] = {};
    ...
    function getLine(doc, n) {
      n -= doc.first;
      if (0 > n || n >= doc.size) {
        throw Error("There is no line " + (n + doc.first) + " in the document.");
      }
      for (; !doc.lines;) {
        for (var i = 0;; ++i) {
          var child = doc.children[i], sz = child.chunkSize();
          if (n < sz) {
            doc = child;
            break;
          }
    ...
    function getBetween(doc, start, end) {
      ...
      doc.iter(start.line, end.line + 1, function(line) {
        line = line.text;
        n == end.line && (line = line.slice(0, end.ch));
        n == start.line && (line = line.slice(start.ch));
        out.push(line);
        ++n;
      });
      return out;
    ...
    function lineAtHeight(chunk, h) {
      ...
      for (i$1 = 0; i$1 < chunk.lines.length; ++i$1) {
        child = chunk.lines[i$1].height;
        if (h < child) {
          break;
        }
        h -= child;
    ...
    function clipPos(doc, pos) {
      ...
      doc = getLine(doc, pos.line).text.length;
      last = pos.ch;
      pos = null == last || last > doc ? Pos(pos.line, doc) : 0 > last ? Pos(pos.line, 0) : pos;
      return pos;
    }
        ...
        runMode(cm, line.text, overlay.mode, context, function(end, style) {
          ...
          if (style) {
            if (overlay.opaque) {
              ...
              for (; start < i; start += 2) {
                end = st[start + 1], st[start + 1] = (end ? end + " " : "") + "overlay " + style;
              }
            }
          }
        }, lineClasses);
    ...
    function processLine(cm, text, context, startAt) {
      ...
      cm = new StringStream(text, cm.options.tabSize, context);
      cm.start = cm.pos = startAt || 0;
      for ("" == text && callBlankLine(mode, context.state); !cm.eol();) {
        readToken(mode, cm, context.state), cm.start = cm.pos;
      }
    }
    function callBlankLine(mode, state) {
      ...
      if (mode.innerMode && (mode = innerMode(mode, state), mode.mode.blankLine)) {
        return mode.mode.blankLine(mode.state);
      }
    }
    ...
    function takeToken(cm, pos, precise, asArray) {
      var doc = cm.doc, mode = doc.mode;
      pos = clipPos(doc, pos);
      var line = getLine(doc, pos.line);
      precise = getContextBefore(cm, pos.line, precise);
      cm = new StringStream(line.text, cm.options.tabSize, precise);
      var tokens;
      for (asArray && (tokens = []); (asArray || cm.pos < pos.ch) && !cm.eol();) {
        cm.start = cm.pos;
        var style = readToken(mode, cm, precise.state);
        asArray && tokens.push(new Token(cm, style, copyState(doc.mode, precise.state)));
      }
      return asArray ? tokens : new Token(cm, style, precise.state);
    }
    ...
    function runMode(cm, text, mode, context, f, lineClasses, forceToEnd) {
      ...
      var curStart = 0, curStyle = null, stream = new StringStream(text, cm.options.tabSize, context), inner = cm.options.addModeClass && [null];
      for ("" == text && extractLineClasses(callBlankLine(mode, context.state), lineClasses); !stream.eol();) {
        ...
        if (!flattenSpans || curStyle != style) {
          for (; curStart < stream.start;) {
            curStart = Math.min(stream.start, curStart + 5000), f(curStart, curStyle);
          }
          curStyle = style;
        }
      ...
      for (; curStart < stream.pos;) {
        cm = Math.min(stream.pos, curStart + 5000), f(cm, curStyle), curStart = cm;
      }
    }
    function findStartLine(cm, n, precise) {
      for (var minindent, minline, doc = cm.doc, lim = precise ? -1 : n - (cm.doc.mode.innerMode ? 1000 : 100); n > lim; --n) {
        if (n <= doc.first) {
          return doc.first;
        }
        var line = getLine(doc, n - 1), after = line.stateAfter;
        if (after && (!precise || n + (after instanceof SavedContext ? after.lookAhead : 0) <= doc.modeFrontier)) {
          return n;
        }
        line = countColumn(line.text, null, cm.options.tabSize);
        if (null == minline || minindent > line) {
          minline = n - 1, minindent = line;
        }
      }
      return minline;
    }
    ...
    function stretchSpansOverChange(doc, change) {
      ...
      doc = change.from.ch;
      var endCh = change.to.ch, isInsert = 0 == cmp(change.from, change.to), nw;
      if (oldFirst) {
        for (var i = 0; i < oldFirst.length; ++i) {
          var span = oldFirst[i], marker = span.marker;
          if (null == span.from || (marker.inclusiveLeft ? span.from <= doc : span.from < doc) || !(span.from != doc || "bookmark" != marker.type || isInsert && span.marker.insertLeft)) {
            var endsAfter = null == span.to || (marker.inclusiveRight ? span.to >= doc : span.to > doc);
      ...
      var last;
      if (oldLast) {
        for (nw = 0; nw < oldLast.length; ++nw) {
          if (i = oldLast[nw], span = i.marker, null == i.to || (span.inclusiveRight ? i.to >= endCh : i.to > endCh) || i.from == endCh && "bookmark" == span.type && (!isInsert || i.marker.insertLeft)) {
            marker = null == i.from || (span.inclusiveLeft ? i.from <= endCh : i.from < endCh), (last || (last = [])).push(new MarkedSpan(span, marker ? null : i.from - endCh, null == i.to ? null : i.to - endCh));
          }
        }
      }
      oldLast = 1 == change.text.length;
      endCh = lst(change.text).length + (oldLast ? doc : 0);
      if (oldFirst) {
        for (isInsert = 0; isInsert < oldFirst.length; ++isInsert) {
          if (nw = oldFirst[isInsert], null == nw.to) {
            (i = getMarkedSpanFor(last, nw.marker), i) ? oldLast && (nw.to = null == i.to ? null : i.to + endCh) : nw.to = doc;
          }
        }
      }
      if (last) {
        for (doc = 0; doc < last.length; ++doc) {
          isInsert = last[doc], null != isInsert.to && (isInsert.to += endCh), null == isInsert.from ? getMarkedSpanFor(oldFirst, isInsert.marker) || (isInsert.from = endCh, oldLast && (oldFirst || (oldFirst = [])).push(isInsert)) : (isInsert.from += endCh, oldLast && (oldFirst || (oldFirst = [])).push(isInsert));
        }
      }
      oldFirst && (oldFirst = clearEmptySpans(oldFirst));
      last && last != oldFirst && (last = clearEmptySpans(last));
      doc = [oldFirst];
      if (!oldLast) {
        change = change.text.length - 2;
        var gapMarkers;
        if (0 < change && oldFirst) {
          for (oldLast = 0; oldLast < oldFirst.length; ++oldLast) {
            null == oldFirst[oldLast].to && (gapMarkers || (gapMarkers = [])).push(new MarkedSpan(oldFirst[oldLast].marker, null, null));
          }
        }
        for (oldFirst = 0; oldFirst < change; ++oldFirst) {
          doc.push(gapMarkers);
        }
        doc.push(last);
      }
    ...
    function removeReadOnlyRanges(doc, from, to) {
      ...
      doc = [{from, to}];
      for (from = 0; from < markers.length; ++from) {
        to = markers[from];
        for (var m = to.find(0), j = 0; j < doc.length; ++j) {
          var p = doc[j];
          if (!(0 > cmp(p.to, m.from) || 0 < cmp(p.from, m.to))) {
            var newParts = [j, 1], dfrom = cmp(p.from, m.from), dto = cmp(p.to, m.to);
            (0 > dfrom || !to.inclusiveLeft && !dfrom) && newParts.push({from:p.from, to:m.from});
            (0 < dto || !to.inclusiveRight && !dto) && newParts.push({from:m.to, to:p.to});
            doc.splice.apply(doc, newParts);
            j += newParts.length - 3;
          }
        }
      }
    ...
    function compareCollapsedMarkers(a, b) {
      ...
      lenDiff = a.find();
      var bPos = b.find(), fromCmp = cmp(lenDiff.from, bPos.from) || (a.inclusiveLeft ? -1 : 0) - (b.inclusiveLeft ? -1 : 0);
      return fromCmp ? -fromCmp : (lenDiff = cmp(lenDiff.to, bPos.to) || (a.inclusiveRight ? 1 : 0) - (b.inclusiveRight ? 1 : 0)) ? lenDiff : b.id - a.id;
    }
    ...
    function conflictingCollapsedRange(doc, lineNo, from, to, marker) {
      doc = getLine(doc, lineNo);
      if (doc = sawCollapsedSpans && doc.markedSpans) {
        for (lineNo = 0; lineNo < doc.length; ++lineNo) {
          var sp = doc[lineNo];
    ...
    function visualLineNo(doc, lineN) {
      doc = getLine(doc, lineN);
      var vis = visualLine(doc);
    ...
    function lineIsHiddenInner(doc, line, span) {
      if (null == span.to) {
        return line = span.marker.find(1, !0), lineIsHiddenInner(doc, line.line, getMarkedSpanFor(line.line.markedSpans, span.marker));
      }
    ...
    function heightAtLine(lineObj) {
      ...
      for (lineObj = chunk.parent; lineObj; chunk = lineObj, lineObj = chunk.parent) {
        for (i = 0; i < lineObj.children.length && (line = lineObj.children[i], line != chunk); ++i) {
          h += line.height;
        }
      }
      return h;
    ...
    function lineLength(line) {
      ...
      for (var len = line.text.length, merged, cur = line; merged = collapsedSpanAtSide(cur, !0);) {
        merged = merged.find(0, !0), cur = merged.from.line, len += merged.from.ch - merged.to.ch;
      }
      for (cur = line; merged = collapsedSpanAtSide(cur, !1);) {
        line = merged.find(0, !0), len -= cur.text.length - line.from.ch, cur = line.to.line, len += cur.text.length - line.to.ch;
      }
      return len;
    }
    function findMaxLine(cm) {
      var d = cm.display;
      cm = cm.doc;
      d.maxLine = getLine(cm, cm.first);
      d.maxLineLength = lineLength(d.maxLine);
      d.maxLineChanged = !0;
      cm.iter(function(line) {
    ...
    function interpretTokenStyle(style, options) {
      ...
      options = options.addModeClass ? styleToClassCacheWithMode : styleToClassCache;
      return options[style] || (options[style] = style.replace(/\S+/g, "cm-$\x26"));
    }
    function buildLineContent(cm, lineView) {
      var content = eltP("span", null, null, webkit ? "padding-right: .1px" : null);
      content = {pre:eltP("pre", [content], "CodeMirror-line"), content, col:0, pos:0, cm, trailingSpace:!1, splitSpaces:cm.getOption("lineWrapping")};
      lineView.measure = {};
      for (var i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
        var line = i ? lineView.rest[i - 1] : lineView.line, order = void 0;
        content.pos = 0;
        content.addToken = buildToken;
        var JSCompiler_inline_result = cm.display.measure;
        if (null != badBidiRects) {
          JSCompiler_inline_result = badBidiRects;
        } else {
          var txt = removeChildrenAndAdd(JSCompiler_inline_result, document.createTextNode("AخA")), r0 = range(txt, 0, 1).getBoundingClientRect();
          txt = range(txt, 1, 2).getBoundingClientRect();
          removeChildren(JSCompiler_inline_result);
          JSCompiler_inline_result = r0 && r0.left != r0.right ? badBidiRects = 3 > txt.right - r0.right : !1;
        }
        JSCompiler_inline_result && (order = getOrder(line, cm.doc.direction)) && (content.addToken = buildTokenBadBidi(content.addToken, order));
        content.map = [];
        var allowFrontierUpdate = lineView != cm.display.externalMeasured && lineNo(line);
        a: {
          var spanEndStyle = txt = r0 = JSCompiler_inline_result = void 0, spanStyle = void 0, css = void 0, style = void 0;
          order = content;
          allowFrontierUpdate = getLineStyles(cm, line, allowFrontierUpdate);
          var spans = line.markedSpans, allText = line.text, at = 0;
          if (spans) {
            for (var len = allText.length, pos = 0, i$jscomp$0 = 1, text = "", nextChange = 0;;) {
              if (nextChange == pos) {
                spanStyle = spanEndStyle = txt = css = "";
                r0 = JSCompiler_inline_result = null;
                nextChange = Infinity;
                for (var foundBookmarks = [], endStyles = void 0, j = 0; j < spans.length; ++j) {
                  ...
                  if ("bookmark" == m.type && sp.from == pos && m.widgetNode) {
                    ...
                    null != sp.to && sp.to != pos && nextChange > sp.to && (nextChange = sp.to, spanEndStyle = "");
                    m.className && (spanStyle += " " + m.className);
                    m.css && (css = (css ? css + ";" : "") + m.css);
                    m.startStyle && sp.from == pos && (txt += " " + m.startStyle);
                    m.endStyle && sp.to == nextChange && (endStyles || (endStyles = [])).push(m.endStyle, sp.to);
                    m.title && ((JSCompiler_inline_result || (JSCompiler_inline_result = {})).title = m.title);
                    if (m.attributes) {
                      for (var attr in m.attributes) {
                        (JSCompiler_inline_result || (JSCompiler_inline_result = {}))[attr] = m.attributes[attr];
                      }
                    }
                    m.collapsed && (!r0 || 0 > compareCollapsedMarkers(r0.marker, m)) && (r0 = sp);
                  } else {
                    sp.from > pos && nextChange > sp.from && (nextChange = sp.from);
                  }
                }
                if (endStyles) {
                  for (j = 0; j < endStyles.length; j += 2) {
                    endStyles[j + 1] == nextChange && (spanEndStyle += " " + endStyles[j]);
                  }
                }
                if (!r0 || r0.from == pos) {
                  for (endStyles = 0; endStyles < foundBookmarks.length; ++endStyles) {
                    buildCollapsedSpan(order, 0, foundBookmarks[endStyles]);
                  }
                }
                if (r0 && (r0.from || 0) == pos) {
                  buildCollapsedSpan(order, (null == r0.to ? len + 1 : r0.to) - pos, r0.marker, null == r0.from);
                  if (null == r0.to) {
                    break a;
                  }
                  r0.to == pos && (r0 = !1);
              ...
              for (foundBookmarks = Math.min(len, nextChange);;) {
                if (text) {
                  endStyles = pos + text.length;
                  r0 || (j = endStyles > foundBookmarks ? text.slice(0, foundBookmarks - pos) : text, order.addToken(order, j, style ? style + spanStyle : spanStyle, txt, pos + j.length == nextChange ? spanEndStyle : "", css, JSCompiler_inline_result));
                  if (endStyles >= foundBookmarks) {
                    text = text.slice(foundBookmarks - pos);
                    pos = foundBookmarks;
                    break;
                  }
                  pos = endStyles;
                ...
                text = allText.slice(at, at = allowFrontierUpdate[i$jscomp$0++]);
                style = interpretTokenStyle(allowFrontierUpdate[i$jscomp$0++], order.cm.options);
              }
            }
          } else {
            for (JSCompiler_inline_result = 1; JSCompiler_inline_result < allowFrontierUpdate.length; JSCompiler_inline_result += 2) {
              order.addToken(order, allText.slice(at, at = allowFrontierUpdate[JSCompiler_inline_result]), interpretTokenStyle(allowFrontierUpdate[JSCompiler_inline_result + 1], order.cm.options));
            }
          }
        }
        line.styleClasses && (line.styleClasses.bgClass && (content.bgClass = joinClasses(line.styleClasses.bgClass, content.bgClass || "")), line.styleClasses.textClass && (content.textClass = joinClasses(line.styleClasses.textClass, content.textClass || "")));
        0 == content.map.length && (line = content.map, order = line.push, JSCompiler_inline_result = content.content, r0 = JSCompiler_inline_result.appendChild, txt = cm.display.measure, null == zwspSupported && (spanEndStyle = elt("span", "​"), removeChildrenAndAdd(txt, elt("span", [spanEndStyle, document.createTextNode("x")])), 0 != txt.firstChild.offsetHeight && (zwspSupported = 1 >= spanEndStyle.offsetWidth && 2 < spanEndStyle.offsetHeight && !(ie && 8 > ie_version))), txt = zwspSupported ? 
        elt("span", "​") : elt("span", " ", null, "display: inline-block; width: 1px; margin-right: -1px"), txt.setAttribute("cm-text", ""), order.call(line, 0, 0, r0.call(JSCompiler_inline_result, txt)));
        0 == i ? (lineView.measure.map = content.map, lineView.measure.cache = {}) : ((lineView.measure.maps || (lineView.measure.maps = [])).push(content.map), (lineView.measure.caches || (lineView.measure.caches = [])).push({}));
      }
      webkit && (attr = content.content.lastChild, /\bcm-tab\b/.test(attr.className) || attr.querySelector && attr.querySelector(".cm-tab")) && (content.content.className = "cm-tab-wrap-hack");
      signal(cm, "renderLine", cm, lineView.line, content.pre);
      content.pre.className && (content.textClass = joinClasses(content.pre.className, content.textClass || ""));
      return content;
    }
    ...
    function buildToken(builder, text, style, startStyle, endStyle, css, attributes) {
      if (text) {
        if (builder.splitSpaces) {
          var JSCompiler_temp = builder.trailingSpace;
          if (1 < text.length && !/  /.test(text)) {
            JSCompiler_temp = text;
          } else {
            for (var result = "", i = 0; i < text.length; i++) {
              var ch = text.charAt(i);
              " " != ch || !JSCompiler_temp || i != text.length - 1 && 32 != text.charCodeAt(i + 1) || (ch = " ");
              result += ch;
              JSCompiler_temp = " " == ch;
            }
        ...
        result = JSCompiler_temp;
        i = builder.cm.state.specialChars;
        ch = !1;
        if (i.test(text)) {
          JSCompiler_temp = document.createDocumentFragment();
          for (var pos = 0;;) {
            i.lastIndex = pos;
            var m = i.exec(text), skipped = m ? m.index - pos : text.length - pos;
            if (skipped) {
              var txt = document.createTextNode(result.slice(pos, pos + skipped));
              ie && 9 > ie_version ? JSCompiler_temp.appendChild(elt("span", [txt])) : JSCompiler_temp.appendChild(txt);
            ...
            pos += skipped + 1;
            "\t" == m[0] ? (m = builder.cm.options.tabSize, m -= builder.col % m, skipped = JSCompiler_temp.appendChild(elt("span", spaceStr(m), "cm-tab")), skipped.setAttribute("role", "presentation"), skipped.setAttribute("cm-text", "\t"), builder.col += m) : ("\r" == m[0] || "\n" == m[0] ? (skipped = JSCompiler_temp.appendChild(elt("span", "\r" == m[0] ? "␍" : "␤", "cm-invalidchar")), skipped.setAttribute("cm-text", m[0])) : (skipped = builder.cm.options.specialCharPlaceholder(m[0]), skipped.setAttribute("cm-text", 
            m[0]), ie && 9 > ie_version ? JSCompiler_temp.appendChild(elt("span", [skipped])) : JSCompiler_temp.appendChild(skipped)), builder.col += 1);
            builder.map.push(builder.pos, builder.pos + 1, skipped);
            builder.pos++;
          }
        } else {
          builder.col += text.length, JSCompiler_temp = document.createTextNode(result), builder.map.push(builder.pos, builder.pos + text.length, JSCompiler_temp), ie && 9 > ie_version && (ch = !0), builder.pos += text.length;
        }
        builder.trailingSpace = 32 == result.charCodeAt(text.length - 1);
        if (style || startStyle || endStyle || ch || css || attributes) {
          text = style || "";
          startStyle && (text += startStyle);
          endStyle && (text += endStyle);
          startStyle = elt("span", [JSCompiler_temp], text, css);
          if (attributes) {
            for (var attr in attributes) {
              attributes.hasOwnProperty(attr) && "style" != attr && "class" != attr && startStyle.setAttribute(attr, attributes[attr]);
            }
          }
          return builder.content.appendChild(startStyle);
        }
        builder.content.appendChild(JSCompiler_temp);
      }
    }
    ...
    function LineView(doc, line$jscomp$0, lineN) {
      for (var line = this.line = line$jscomp$0, lines; line = collapsedSpanAtSide(line, !1);) {
        line = line.find(1, !0).line, (lines || (lines = [])).push(line);
      }
      this.size = (this.rest = lines) ? lineNo(lst(this.rest)) - lineN + 1 : 1;
      this.node = this.text = null;
      this.hidden = lineIsHidden(doc, line$jscomp$0);
    }
    function buildViewArray(cm, from, to) {
      var array = [], nextPos;
      for (nextPos = from; nextPos < to;) {
        from = new LineView(cm.doc, getLine(cm.doc, nextPos), nextPos), nextPos += from.size, array.push(from);
      }
      return array;
    }
    function finishOperation(op$jscomp$0, endCb) {
      if (op$jscomp$0 = op$jscomp$0.ownsGroup) {
        try {
          var callbacks = op$jscomp$0.delayedCallbacks, i = 0;
          do {
            for (; i < callbacks.length; i++) {
              callbacks[i].call(null);
            }
            for (var j = 0; j < op$jscomp$0.ops.length; j++) {
              var op = op$jscomp$0.ops[j];
          ...
          operationGroup = null, endCb(op$jscomp$0);
        }
      }
    }
    ...
    function updateLineForChanges(cm, lineView$jscomp$0, lineN, dims) {
      for (var j = 0; j < lineView$jscomp$0.changes.length; j++) {
        var type = lineView$jscomp$0.changes[j];
        if ("text" == type) {
          type = cm;
          var lineView = lineView$jscomp$0, cls = lineView.text.className, built = getLineContent(type, lineView);
          lineView.text == lineView.node && (lineView.node = built.pre);
          lineView.text.parentNode.replaceChild(built.pre, lineView.text);
          lineView.text = built.pre;
          built.bgClass != lineView.bgClass || built.textClass != lineView.textClass ? (lineView.bgClass = built.bgClass, lineView.textClass = built.textClass, updateLineClasses(type, lineView)) : cls && (lineView.text.className = cls);
        } else if ("gutter" == type) {
          updateLineGutter(cm, lineView$jscomp$0, lineN, dims);
        } else if ("class" == type) {
          updateLineClasses(cm, lineView$jscomp$0);
        } else if ("widget" == type) {
          type = cm;
          lineView = lineView$jscomp$0;
          cls = dims;
          lineView.alignable && (lineView.alignable = null);
          built = classTest("CodeMirror-linewidget");
          for (var node = lineView.node.firstChild, next; node; node = next) {
            next = node.nextSibling, built.test(node.className) && lineView.node.removeChild(node);
          }
          insertLineWidgets(type, lineView, cls);
        }
      }
      lineView$jscomp$0.changes = null;
    }
    function ensureLineWrapped(lineView) {
      lineView.node == lineView.text && (lineView.node = elt("div", null, null, "position: relative"), lineView.text.parentNode && lineView.text.parentNode.replaceChild(lineView.node, lineView.text), lineView.node.appendChild(lineView.text), ie && 8 > ie_version && (lineView.node.style.zIndex = 2));
    ...
    function updateLineGutter(cm, lineView, lineN, dims) {
      ...
      lineView.gutterBackground && (lineView.node.removeChild(lineView.gutterBackground), lineView.gutterBackground = null);
      ...
      wrap = lineView.line.gutterMarkers;
      if (cm.options.lineNumbers || wrap) {
        ...
        !cm.options.lineNumbers || wrap && wrap["CodeMirror-linenumbers"] || (lineView.lineNumber = gutterWrap.appendChild(elt("div", lineNumberFor(cm.options, lineN), "CodeMirror-linenumber CodeMirror-gutter-elt", "left: " + dims.gutterLeft["CodeMirror-linenumbers"] + "px; width: " + cm.display.lineNumInnerWidth + "px")));
        if (wrap) {
          for (lineView = 0; lineView < cm.display.gutterSpecs.length; ++lineView) {
            lineN = cm.display.gutterSpecs[lineView].className, (wrap$1 = wrap.hasOwnProperty(lineN) && wrap[lineN]) && gutterWrap.appendChild(elt("div", [wrap$1], "CodeMirror-gutter-elt", "left: " + dims.gutterLeft[lineN] + "px; width: " + dims.gutterWidth[lineN] + "px"));
          }
        }
      }
    }
    ...
    function insertLineWidgetsFor(cm, line, lineView, dims$jscomp$0, allowAbove) {
      if (line.widgets) {
        ...
        for (line = line.widgets; i < line.length; ++i) {
          var widget = line[i], node = elt("div", [widget.node], "CodeMirror-linewidget" + (widget.className ? " " + widget.className : ""));
    ...
    function eventInWidget(display, e) {
      for (e = e.target || e.srcElement; e != display.wrapper; e = e.parentNode) {
        if (!e || 1 == e.nodeType && "true" == e.getAttribute("cm-ignore-events") || e.parentNode == display.sizer && e != display.mover) {
          return !0;
        }
      }
    }
    ...
    function paddingH(display) {
      ...
      e = {left:parseInt(e.paddingLeft), right:parseInt(e.paddingRight)};
    ...
    function mapFromLineView(lineView, line, lineN) {
      ...
      if (lineView.rest) {
        ...
        for (line = 0; line < lineView.rest.length; line++) {
          if (lineNo(lineView.rest[line]) > lineN) {
            return {map:lineView.measure.maps[line], cache:lineView.measure.caches[line], before:!0};
          }
        }
      }
    }
    function findViewForLine(cm, lineN) {
      ...
      if ((cm = cm.display.externalMeasured) && lineN >= cm.lineN && lineN < cm.lineN + cm.size) {
        return cm;
      }
    }
    function prepareMeasureForLine(cm, line$jscomp$0) {
      var lineN = lineNo(line$jscomp$0), view = findViewForLine(cm, lineN);
      view && !view.text ? view = null : view && view.changes && (updateLineForChanges(cm, view, lineN, getDimensions(cm)), cm.curOp.forceUpdate = !0);
      if (!view) {
        var line = visualLine(line$jscomp$0);
        view = lineNo(line);
        line = cm.display.externalMeasured = new LineView(cm.doc, line, view);
        line.lineN = view;
        view = line.built = buildLineContent(cm, line);
        line.text = view.pre;
        removeChildrenAndAdd(cm.display.lineMeasure, view.pre);
        view = line;
      }
      cm = mapFromLineView(view, line$jscomp$0, lineN);
      return {line:line$jscomp$0, view, rect:null, map:cm.map, cache:cm.cache, before:cm.before, hasHeights:!1};
    }
    function measureCharPrepared(cm, prepared, ch, bias, varHeight) {
      prepared.before && (ch = -1);
      var key = ch + (bias || "");
      if (prepared.cache.hasOwnProperty(key)) {
        cm = prepared.cache[key];
      } else {
        ...
        if (!prepared.hasHeights) {
          ...
          if (!lineView.measure.heights || wrapping && lineView.measure.width != curWidth) {
            ...
            if (wrapping) {
              for (lineView.measure.width = curWidth, lineView = lineView.text.firstChild.getClientRects(), wrapping = 0; wrapping < lineView.length - 1; wrapping++) {
                curWidth = lineView[wrapping];
                var next = lineView[wrapping + 1];
                2 < Math.abs(curWidth.bottom - next.bottom) && heights.push((curWidth.bottom + next.top) / 2 - rect.top);
              }
            }
        ...
        heights = bias;
        lineView = nodeAndOffsetInLineMap(prepared.map, ch, heights);
        bias = lineView.node;
        rect = lineView.start;
        wrapping = lineView.end;
        ch = lineView.collapse;
        if (3 == bias.nodeType) {
          for (var i$1 = 0; 4 > i$1; i$1++) {
            for (; rect && isExtendingChar(prepared.line.text.charAt(lineView.coverStart + rect));) {
              --rect;
            }
            for (; lineView.coverStart + wrapping < lineView.coverEnd && isExtendingChar(prepared.line.text.charAt(lineView.coverStart + wrapping));) {
              ++wrapping;
            }
            if (ie && 9 > ie_version && 0 == rect && wrapping == lineView.coverEnd - lineView.coverStart) {
              var rect$jscomp$0 = bias.parentNode.getBoundingClientRect();
            } else {
              rect$jscomp$0 = range(bias, rect, wrapping).getClientRects();
              wrapping = nullRect;
              if ("left" == heights) {
                for (curWidth = 0; curWidth < rect$jscomp$0.length && (wrapping = rect$jscomp$0[curWidth]).left == wrapping.right; curWidth++) {
                }
              } else {
                for (curWidth = rect$jscomp$0.length - 1; 0 <= curWidth && (wrapping = rect$jscomp$0[curWidth]).left == wrapping.right; curWidth--) {
                }
              }
              rect$jscomp$0 = wrapping;
            }
            if (rect$jscomp$0.left || rect$jscomp$0.right || 0 == rect) {
              break;
            }
            wrapping = rect;
            --rect;
          ...
          ie && 11 > ie_version && ((i$1 = !window.screen || null == screen.logicalXDPI || screen.logicalXDPI == screen.deviceXDPI) || (null != badZoomedRects ? i$1 = badZoomedRects : (heights = removeChildrenAndAdd(cm.display.measure, elt("span", "x")), i$1 = heights.getBoundingClientRect(), heights = range(heights, 0, 1).getBoundingClientRect(), i$1 = badZoomedRects = 1 < Math.abs(i$1.left - heights.left)), i$1 = !i$1), i$1 || (i$1 = screen.logicalXDPI / screen.deviceXDPI, heights = screen.logicalYDPI / 
          screen.deviceYDPI, rect$jscomp$0 = {left:rect$jscomp$0.left * i$1, right:rect$jscomp$0.right * i$1, top:rect$jscomp$0.top * heights, bottom:rect$jscomp$0.bottom * heights}));
        } else {
          0 < rect && (ch = heights = "right"), rect$jscomp$0 = cm.options.lineWrapping && 1 < (i$1 = bias.getClientRects()).length ? i$1["right" == heights ? i$1.length - 1 : 0] : bias.getBoundingClientRect();
        }
        !(ie && 9 > ie_version) || rect || rect$jscomp$0 && (rect$jscomp$0.left || rect$jscomp$0.right) || (rect$jscomp$0 = (rect$jscomp$0 = bias.parentNode.getClientRects()[0]) ? {left:rect$jscomp$0.left, right:rect$jscomp$0.left + charWidth(cm.display), top:rect$jscomp$0.top, bottom:rect$jscomp$0.bottom} : nullRect);
        bias = rect$jscomp$0.top - prepared.rect.top;
        rect = rect$jscomp$0.bottom - prepared.rect.top;
        i$1 = (bias + rect) / 2;
        heights = prepared.view.measure.heights;
        for (lineView = 0; lineView < heights.length - 1 && !(i$1 < heights[lineView]); lineView++) {
        }
        ch = {left:("right" == ch ? rect$jscomp$0.right : rect$jscomp$0.left) - prepared.rect.left, right:("left" == ch ? rect$jscomp$0.left : rect$jscomp$0.right) - prepared.rect.left, top:lineView ? heights[lineView - 1] : 0, bottom:heights[lineView]};
        rect$jscomp$0.left || rect$jscomp$0.right || (ch.bogus = !0);
        cm.options.singleCursorHeightPerLine || (ch.rtop = bias, ch.rbottom = rect);
        cm = ch;
        cm.bogus || (prepared.cache[key] = cm);
      }
      return {left:cm.left, right:cm.right, top:varHeight ? cm.rtop : cm.top, bottom:varHeight ? cm.rbottom : cm.bottom};
    }
    ...
    function pageScrollX(doc) {
      return chrome && android ? -(doc.body.getBoundingClientRect().left - parseInt(getComputedStyle(doc.body).marginLeft)) : doc.defaultView.pageXOffset || (doc.documentElement || doc.body).scrollLeft;
    }
    function pageScrollY(doc) {
      return chrome && android ? -(doc.body.getBoundingClientRect().top - parseInt(getComputedStyle(doc.body).marginTop)) : doc.defaultView.pageYOffset || (doc.documentElement || doc.body).scrollTop;
    }
    ...
    function intoCoordSystem(cm, lineObj, rect, context, includeWidgets) {
      includeWidgets || (includeWidgets = widgetTopHeight(lineObj), rect.top += includeWidgets, rect.bottom += includeWidgets);
      if ("line" == context) {
        return rect;
      }
      ...
      if ("page" == context || "window" == context) {
        includeWidgets = cm.display.lineSpace.getBoundingClientRect(), lineObj += includeWidgets.top + ("window" == context ? 0 : pageScrollY(doc(cm))), cm = includeWidgets.left + ("window" == context ? 0 : pageScrollX(doc(cm))), rect.left += cm, rect.right += cm;
      }
      rect.top += lineObj;
      rect.bottom += lineObj;
      return rect;
    ...
    function fromCoordSystem(cm, coords, context) {
      ...
      coords = coords.top;
      "page" == context ? (left -= pageScrollX(doc(cm)), coords -= pageScrollY(doc(cm))) : "local" != context && context || (context = cm.display.sizer.getBoundingClientRect(), left += context.left, coords += context.top);
      cm = cm.display.lineSpace.getBoundingClientRect();
      return {left:left - cm.left, top:coords - cm.top};
    }
    function charCoords(cm, pos, context, lineObj, bias) {
      lineObj || (lineObj = getLine(cm.doc, pos.line));
      var JSCompiler_temp_const = lineObj;
      pos = pos.ch;
      lineObj = measureCharPrepared(cm, prepareMeasureForLine(cm, lineObj), pos, bias);
      return intoCoordSystem(cm, JSCompiler_temp_const, lineObj, context);
    }
    function cursorCoords(cm, pos, context, lineObj, preparedMeasure, varHeight) {
      function get(ch, right) {
        ch = measureCharPrepared(cm, preparedMeasure, ch, right ? "right" : "left", varHeight);
        right ? ch.left = ch.right : ch.right = ch.left;
        return intoCoordSystem(cm, lineObj, ch, context);
      }
      ...
      pos = pos.sticky;
      ch >= lineObj.text.length ? (ch = lineObj.text.length, pos = "before") : 0 >= ch && (ch = 0, pos = "after");
      if (!order) {
        return get("before" == pos ? ch - 1 : ch, "before" == pos);
      }
      var partPos = getBidiPartAt(order, ch, pos), other = bidiOther;
      partPos = getBidi(ch, partPos, "before" == pos);
      null != other && (partPos.other = getBidi(ch, other, "before" != pos));
      return partPos;
    }
    function estimateCoords(cm, pos) {
      ...
      pos = getLine(cm.doc, pos.line);
      cm = heightAtLine(pos) + cm.display.lineSpace.offsetTop;
      return {left, right:left, top:cm, bottom:cm + pos.height};
    }
    function PosWithInfo(line, ch, sticky, outside, xRel) {
      line = Pos(line, ch, sticky);
      line.xRel = xRel;
      outside && (line.outside = outside);
      return line;
    }
    function coordsChar(cm, x, y) {
      ...
      for (var lineObj = getLine(doc, lineN);;) {
        last = coordsCharInner(cm, lineObj, lineN, x, y);
        var collapsed = void 0;
        var ch = last.ch + (0 < last.xRel || 0 < last.outside ? 1 : 0);
        if (lineObj = sawCollapsedSpans && lineObj.markedSpans) {
          for (var i = 0; i < lineObj.length; ++i) {
            var sp = lineObj[i];
            sp.marker.collapsed && (null == sp.from || sp.from < ch) && (null == sp.to || sp.to > ch) && (!collapsed || 0 > compareCollapsedMarkers(collapsed, sp.marker)) && (collapsed = sp.marker);
          }
        }
        if (!collapsed) {
          return last;
        }
        last = collapsed.find(1);
        if (last.line == lineN) {
          return last;
        }
        lineObj = getLine(doc, lineN = last.line);
      }
    }
    function wrappedLineExtent(cm, lineObj, preparedMeasure, y) {
      y -= widgetTopHeight(lineObj);
      lineObj = lineObj.text.length;
      var begin = findFirst(function(ch) {
        return measureCharPrepared(cm, preparedMeasure, ch - 1).bottom <= y;
      }, lineObj, 0);
      lineObj = findFirst(function(ch) {
        return measureCharPrepared(cm, preparedMeasure, ch).top > y;
      }, begin, lineObj);
      return {begin, end:lineObj};
    }
    ...
    function coordsCharInner(cm, lineObj, lineNo, x, y) {
      ...
      order && (order = (cm.options.lineWrapping ? coordsBidiPartWrapped : coordsBidiPart)(cm, lineObj, lineNo, preparedMeasure, order, x, y), begin = (ltr = 1 != order.level) ? order.from : order.to - 1, end = ltr ? order.to : order.from - 1);
      var chAround = null, boxAround = null;
      order = findFirst(function(ch) {
      ...
      var outside = !1;
      boxAround ? (begin = x - boxAround.left < boxAround.right - x, ltr = begin == ltr, order = chAround + (ltr ? 0 : 1), ltr = ltr ? "after" : "before", begin = begin ? boxAround.left : boxAround.right) : (ltr || order != end && order != begin || order++, ltr = 0 == order ? "after" : order == lineObj.text.length ? "before" : measureCharPrepared(cm, preparedMeasure, order - (ltr ? 1 : 0)).bottom + widgetHeight <= y == ltr ? "after" : "before", outside = cursorCoords(cm, Pos(lineNo, order, ltr), 
      "line", lineObj, preparedMeasure), begin = outside.left, outside = y < outside.top ? -1 : y >= outside.bottom ? 1 : 0);
      order = skipExtendingChars(lineObj.text, order, 1);
      return PosWithInfo(lineNo, order, ltr, outside, x - begin);
    }
    function coordsBidiPart(cm, lineObj, lineNo, preparedMeasure, order, x, y) {
      var index = findFirst(function(i) {
        i = order[i];
        var ltr = 1 != i.level;
        return boxIsAfter(cursorCoords(cm, Pos(lineNo, ltr ? i.to : i.from, ltr ? "before" : "after"), "line", lineObj, preparedMeasure), x, y, !0);
      }, 0, order.length - 1), part = order[index];
      if (0 < index) {
        var ltr = 1 != part.level;
        ltr = cursorCoords(cm, Pos(lineNo, ltr ? part.from : part.to, ltr ? "after" : "before"), "line", lineObj, preparedMeasure);
        boxIsAfter(ltr, x, y, !0) && ltr.top > y && (part = order[index - 1]);
      }
      return part;
    }
    function coordsBidiPartWrapped(cm, lineObj, _lineNo, preparedMeasure, order, x, y) {
      y = wrappedLineExtent(cm, lineObj, preparedMeasure, y);
      _lineNo = y.begin;
      y = y.end;
      /\s/.test(lineObj.text.charAt(y - 1)) && y--;
      for (var closestDist = lineObj = null, i = 0; i < order.length; i++) {
        var p = order[i];
        if (!(p.from >= y || p.to <= _lineNo)) {
          var endX = measureCharPrepared(cm, preparedMeasure, 1 != p.level ? Math.min(y, p.to) - 1 : Math.max(_lineNo, p.from)).right;
          endX = endX < x ? x - endX + 1e9 : endX - x;
          if (!lineObj || closestDist > endX) {
            lineObj = p, closestDist = endX;
          }
        }
      }
      lineObj || (lineObj = order[order.length - 1]);
      lineObj.from < _lineNo && (lineObj = {from:_lineNo, to:lineObj.to, level:lineObj.level});
      lineObj.to > y && (lineObj = {from:lineObj.from, to:y, level:lineObj.level});
      return lineObj;
    }
    function textHeight(display) {
      ...
      i = measureText.offsetHeight / 50;
      3 < i && (display.cachedTextHeight = i);
    ...
    function charWidth(display) {
      ...
      anchor = (anchor.right - anchor.left) / 10;
      2 < anchor && (display.cachedCharWidth = anchor);
      return anchor || 10;
    }
    ...
    function posFromMouse(cm, e, liberal, forRect) {
      ...
      if (!liberal && "true" == (e.target || e.srcElement).getAttribute("cm-not-content")) {
        return null;
      }
      liberal = display.lineSpace.getBoundingClientRect();
      try {
        var x = e.clientX - liberal.left;
        var y = e.clientY - liberal.top;
      } catch (e$1) {
        return null;
      }
      e = coordsChar(cm, x, y);
      var line;
      forRect && 0 < e.xRel && (line = getLine(cm.doc, e.line).text).length == e.ch && (forRect = countColumn(line, line.length, cm.options.tabSize) - line.length, e = Pos(e.line, Math.max(0, Math.round((x - paddingH(cm.display).left) / charWidth(cm.display)) - forRect)));
      return e;
    }
    ...
    function regChange(cm, from, to, lendiff) {
      ...
      if (from >= display.viewTo) {
        ...
        (cut = viewCuttingPoint(cm, from, from, -1)) ? (display.view = display.view.slice(0, cut.index), display.viewTo = cut.lineN) : resetView(cm);
      } else {
        cut = viewCuttingPoint(cm, from, from, -1);
        var cutBot = viewCuttingPoint(cm, to, to + lendiff, 1);
        cut && cutBot ? (display.view = display.view.slice(0, cut.index).concat(buildViewArray(cm, cut.lineN, cutBot.lineN)).concat(display.view.slice(cutBot.index)), display.viewTo += lendiff) : resetView(cm);
      }
      if (cm = display.externalMeasured) {
        to < cm.lineN ? cm.lineN += lendiff : from < cm.lineN + cm.size && (display.externalMeasured = null);
      }
    }
    function regLineChange(cm, line, type) {
      ...
      line < display.viewFrom || line >= display.viewTo || (cm = display.view[findViewIndex(cm, line)], null != cm.node && (cm = cm.changes || (cm.changes = []), -1 == indexOf(cm, type) && cm.push(type)));
    }
    ...
    function viewCuttingPoint(cm, oldN, newN, dir) {
      var index = findViewIndex(cm, oldN), view = cm.display.view;
    ...
    function countDirtyView(cm) {
      cm = cm.display.view;
      for (var dirty = 0, i = 0; i < cm.length; i++) {
        var lineView = cm[i];
    ...
    function drawSelectionCursor(cm, head, output) {
      ...
      /\bcm-fat-cursor\b/.test(cm.getWrapperElement().className) && (head = charCoords(cm, head, "div", null, null), head = head.right - head.left, cursor.style.width = (0 < head ? head : cm.defaultCharWidth()) + "px");
      pos.other && (cm = output.appendChild(elt("div", " ", "CodeMirror-cursor CodeMirror-secondarycursor")), cm.style.display = "", cm.style.left = pos.other.left + "px", cm.style.top = pos.other.top + "px", cm.style.height = .85 * (pos.other.bottom - pos.other.top) + "px");
    }
    ...
    function drawSelectionRange(cm, range, output) {
      ...
      function drawForLine(line, fromArg, toArg) {
        ...
        function wrapX(pos, dir, side) {
          pos = wrappedLineExtentChar(cm, lineObj, null, pos);
          dir = "ltr" == dir == ("after" == side) ? "left" : "right";
          side = "after" == side ? pos.begin : pos.end - (/\s/.test(lineObj.text.charAt(pos.end - 1)) ? 2 : 1);
          return coords(side, dir)[dir];
        }
        ...
        iterateBidiSections(order, fromArg || 0, null == toArg ? lineLen : toArg, function(from, to, dir, i) {
          var ltr = "ltr" == dir, fromPos = coords(from, ltr ? "left" : "right"), toPos = coords(to - 1, ltr ? "right" : "left"), openStart = null == fromArg && 0 == from, openEnd = null == toArg && to == lineLen, first = 0 == i;
          i = !order || i == order.length - 1;
          3 >= toPos.top - fromPos.top ? (to = (docLTR ? openStart : openEnd) && first ? leftSide : (ltr ? fromPos : toPos).left, add(to, fromPos.top, ((docLTR ? openEnd : openStart) && i ? rightSide : (ltr ? toPos : fromPos).right) - to, fromPos.bottom)) : (ltr ? (ltr = docLTR && openStart && first ? leftSide : fromPos.left, openStart = docLTR ? rightSide : wrapX(from, dir, "before"), from = docLTR ? leftSide : wrapX(to, dir, "after"), openEnd = docLTR && openEnd && i ? rightSide : toPos.right) : 
          (ltr = docLTR ? wrapX(from, dir, "before") : leftSide, openStart = !docLTR && openStart && first ? rightSide : fromPos.right, from = !docLTR && openEnd && i ? leftSide : toPos.left, openEnd = docLTR ? wrapX(to, dir, "after") : rightSide), add(ltr, fromPos.top, openStart - ltr, fromPos.bottom), fromPos.bottom < toPos.top && add(leftSide, fromPos.bottom, null, toPos.top), add(from, toPos.top, openEnd - from, toPos.bottom));
      ...
      if (display.line == range.line) {
        ...
        padding = getLine(doc, range.line);
        padding = visualLine(fromLine) == visualLine(padding);
        display = drawForLine(display.line, display.ch, padding ? fromLine.text.length + 1 : null).end;
        range = drawForLine(range.line, padding ? 0 : null, range.ch).start;
        padding && (display.top < range.top - 2 ? (add(display.right, display.top, null, display.bottom), add(leftSide, range.top, range.left, range.bottom)) : add(display.right, display.top, range.left - display.right, display.bottom));
    ...
    function updateHeightsInViewport(cm) {
      for (var display = cm.display, prevBottom = display.lineDiv.offsetTop, viewTop = Math.max(0, display.scroller.getBoundingClientRect().top), oldHeight = display.lineDiv.getBoundingClientRect().top, mustScroll = 0, i = 0; i < display.view.length; i++) {
        var cur = display.view[i], wrapping = cm.options.lineWrapping, width = 0;
        if (!cur.hidden) {
          oldHeight += cur.line.height;
          if (ie && 8 > ie_version) {
            wrapping = cur.node.offsetTop + cur.node.offsetHeight;
            var height = wrapping - prevBottom;
            prevBottom = wrapping;
          } else {
          ...
          wrapping = cur.line.height - height;
          if (.005 < wrapping || -.005 > wrapping) {
            if (oldHeight < viewTop && (mustScroll -= wrapping), updateLineHeight(cur.line, height), updateWidgetHeight(cur.line), cur.rest) {
              for (height = 0; height < cur.rest.length; height++) {
                updateWidgetHeight(cur.rest[height]);
              }
            }
          }
          width > cm.display.sizerWidth && (width = Math.ceil(width / charWidth(cm.display)), width > cm.display.maxLineLength && (cm.display.maxLineLength = width, cm.display.maxLine = cur.line, cm.display.maxLineChanged = !0));
        }
      }
    ...
    function visibleLines(display, doc, viewport) {
      ...
      top = Math.floor(top - display.lineSpace.offsetTop);
      var bottom = viewport && null != viewport.bottom ? viewport.bottom : top + display.wrapper.clientHeight;
      top = lineAtHeight(doc, top);
      bottom = lineAtHeight(doc, bottom);
      if (viewport && viewport.ensure) {
        var ensureFrom = viewport.ensure.from.line;
        viewport = viewport.ensure.to.line;
        ensureFrom < top ? (top = ensureFrom, bottom = lineAtHeight(doc, heightAtLine(getLine(doc, ensureFrom)) + display.wrapper.clientHeight)) : Math.min(viewport, doc.lastLine()) >= bottom && (top = lineAtHeight(doc, heightAtLine(getLine(doc, viewport)) - display.wrapper.clientHeight), bottom = viewport);
      }
      return {from:top, to:Math.max(bottom, top + 1)};
    }
    function calculateScrollPos(cm, rect) {
      ...
      var docBottom = cm.doc.height + paddingVert(display), atTop = rect.top < snapMargin;
      snapMargin = rect.bottom > docBottom - snapMargin;
      rect.top < screentop ? result.scrollTop = atTop ? 0 : rect.top : rect.bottom > screentop + screen && (screen = Math.min(rect.top, (snapMargin ? docBottom : rect.bottom) - screen), screen != screentop && (result.scrollTop = screen));
      screentop = cm.options.fixedGutter ? 0 : display.gutters.offsetWidth;
      screen = cm.curOp && null != cm.curOp.scrollLeft ? cm.curOp.scrollLeft : display.scroller.scrollLeft - screentop;
      cm = displayWidth(cm) - display.gutters.offsetWidth;
      if (display = rect.right - rect.left > cm) {
        rect.right = rect.left + cm;
      }
      10 > rect.left ? result.scrollLeft = 0 : rect.left < screen ? result.scrollLeft = Math.max(0, rect.left + screentop - (display ? 0 : 10)) : rect.right > cm + screen - 3 && (result.scrollLeft = rect.right + (display ? 0 : 10) - cm);
      return result;
    }
    ...
    function updateScrollbars(cm, measure) {
      ...
      for (measure = 0; 4 > measure && startWidth != cm.display.barWidth || startHeight != cm.display.barHeight; measure++) {
    ...
    function startOperation(cm) {
      cm.curOp = {cm, viewChanged:!1, startHeight:cm.doc.height, forceUpdate:!1, updateInput:0, typing:!1, changeObjs:null, cursorActivityHandlers:null, cursorActivityCalled:0, selectionChanged:!1, updateMaxLine:!1, scrollLeft:null, scrollTop:null, scrollToPos:null, focus:!1, id:++nextOpId, markArrays:null};
      cm = cm.curOp;
      operationGroup ? operationGroup.ops.push(cm) : cm.ownsGroup = operationGroup = {ops:[cm], delayedCallbacks:[]};
    }
    function endOperation(cm) {
      (cm = cm.curOp) && finishOperation(cm, function(group) {
        for (var i = 0; i < group.ops.length; i++) {
          group.ops[i].cm.curOp = null;
        }
        group = group.ops;
        for (i = 0; i < group.length; i++) {
          var op = group[i], cm = op.cm, display = cm.display, display$jscomp$0 = cm.display;
          !display$jscomp$0.scrollbarsClipped && display$jscomp$0.scroller.offsetWidth && (display$jscomp$0.nativeBarWidth = display$jscomp$0.scroller.offsetWidth - display$jscomp$0.scroller.clientWidth, display$jscomp$0.heightForcer.style.height = scrollGap(cm) + "px", display$jscomp$0.sizer.style.marginBottom = -display$jscomp$0.nativeBarWidth + "px", display$jscomp$0.sizer.style.borderRightWidth = scrollGap(cm) + "px", display$jscomp$0.scrollbarsClipped = !0);
        ...
        for (i = 0; i < group.length; i++) {
          op = group[i], op.updatedDisplay = op.mustUpdate && updateDisplayIfNeeded(op.cm, op.update);
        }
        for (i = 0; i < group.length; i++) {
          if (op = group[i], cm = op.cm, display = cm.display, op.updatedDisplay && updateHeightsInViewport(cm), op.barMeasure = measureForScrollbars(cm), display.maxLineChanged && !cm.options.lineWrapping && (display$jscomp$0 = display.maxLine.text.length, display$jscomp$0 = measureCharPrepared(cm, prepareMeasureForLine(cm, display.maxLine), display$jscomp$0, void 0), op.adjustWidthTo = display$jscomp$0.left + 3, cm.display.sizerWidth = op.adjustWidthTo, op.barMeasure.scrollWidth = Math.max(display.scroller.clientWidth, 
          display.sizer.offsetLeft + op.adjustWidthTo + scrollGap(cm) + cm.display.barWidth), op.maxScrollLeft = Math.max(0, display.sizer.offsetLeft + op.adjustWidthTo - displayWidth(cm))), op.updatedDisplay || op.selectionChanged) {
            op.preparedSelection = display.input.prepareSelection();
          }
        }
        for (i = 0; i < group.length; i++) {
          op = group[i], cm = op.cm, null != op.adjustWidthTo && (cm.display.sizer.style.minWidth = op.adjustWidthTo + "px", op.maxScrollLeft < cm.doc.scrollLeft && setScrollLeft(cm, Math.min(cm.display.scroller.scrollLeft, op.maxScrollLeft), !0), cm.display.maxLineChanged = !1), display = op.focus && op.focus == activeElt(rootNode(cm.display.wrapper)), op.preparedSelection && cm.display.input.showSelection(op.preparedSelection, display), (op.updatedDisplay || op.startHeight != cm.doc.height) && 
          updateScrollbars(cm, op.barMeasure), op.updatedDisplay && setDocumentHeight(cm, op.barMeasure), op.selectionChanged && restartBlink(cm), cm.state.focused && op.updateInput && cm.display.input.reset(op.typing), display && ensureFocus(op.cm);
        }
        for (i = 0; i < group.length; i++) {
          var rect = void 0;
          op = group[i];
          ...
          if (op.scrollToPos) {
            var pos = clipPos(display$jscomp$0, op.scrollToPos.from);
            var rect$jscomp$0 = clipPos(display$jscomp$0, op.scrollToPos.to);
            var margin = op.scrollToPos.margin;
            null == margin && (margin = 0);
            cm.options.lineWrapping || pos != rect$jscomp$0 || (rect$jscomp$0 = "before" == pos.sticky ? Pos(pos.line, pos.ch + 1, "before") : pos, pos = pos.ch ? Pos(pos.line, "before" == pos.sticky ? pos.ch - 1 : pos.ch, "after") : pos);
            for (var limit = 0; 5 > limit; limit++) {
              var changed = !1;
              rect = cursorCoords(cm, pos);
              var endCoords = rect$jscomp$0 && rect$jscomp$0 != pos ? cursorCoords(cm, rect$jscomp$0) : rect;
              rect = {left:Math.min(rect.left, endCoords.left), top:Math.min(rect.top, endCoords.top) - margin, right:Math.max(rect.left, endCoords.left), bottom:Math.max(rect.bottom, endCoords.bottom) + margin};
              endCoords = calculateScrollPos(cm, rect);
              var startTop = cm.doc.scrollTop, startLeft = cm.doc.scrollLeft;
              null != endCoords.scrollTop && (updateScrollTop(cm, endCoords.scrollTop), 1 < Math.abs(cm.doc.scrollTop - startTop) && (changed = !0));
              null != endCoords.scrollLeft && (setScrollLeft(cm, endCoords.scrollLeft), 1 < Math.abs(cm.doc.scrollLeft - startLeft) && (changed = !0));
              if (!changed) {
                break;
              }
            }
            rect$jscomp$0 = rect;
            signalDOMEvent(cm, "scrollCursorIntoView") || (margin = cm.display, limit = margin.sizer.getBoundingClientRect(), pos = null, changed = margin.wrapper.ownerDocument, 0 > rect$jscomp$0.top + limit.top ? pos = !0 : rect$jscomp$0.bottom + limit.top > (changed.defaultView.innerHeight || changed.documentElement.clientHeight) && (pos = !1), null == pos || phantom || (rect$jscomp$0 = elt("div", "​", null, "position: absolute;\n                         top: " + (rect$jscomp$0.top - margin.viewOffset - 
            cm.display.lineSpace.offsetTop) + "px;\n                         height: " + (rect$jscomp$0.bottom - rect$jscomp$0.top + scrollGap(cm) + margin.barHeight) + "px;\n                         left: " + rect$jscomp$0.left + "px; width: " + Math.max(2, rect$jscomp$0.right - rect$jscomp$0.left) + "px;"), cm.display.lineSpace.appendChild(rect$jscomp$0), rect$jscomp$0.scrollIntoView(pos), cm.display.lineSpace.removeChild(rect$jscomp$0)));
          }
          rect$jscomp$0 = op.maybeHiddenMarkers;
          pos = op.maybeUnhiddenMarkers;
          if (rect$jscomp$0) {
            for (margin = 0; margin < rect$jscomp$0.length; ++margin) {
              rect$jscomp$0[margin].lines.length || signal(rect$jscomp$0[margin], "hide");
            }
          }
          if (pos) {
            for (rect$jscomp$0 = 0; rect$jscomp$0 < pos.length; ++rect$jscomp$0) {
              pos[rect$jscomp$0].lines.length && signal(pos[rect$jscomp$0], "unhide");
            }
          }
          display.wrapper.offsetHeight && (display$jscomp$0.scrollTop = cm.display.scroller.scrollTop);
      ...
      if (!(doc.highlightFrontier >= cm.display.viewTo)) {
        ...
        doc.iter(context.line, Math.min(doc.first + doc.size, cm.display.viewTo + 500), function(line) {
          if (context.line >= cm.display.viewFrom) {
            ...
            resetState = line.styleClasses;
            (highlighted = highlighted.classes) ? line.styleClasses = highlighted : resetState && (line.styleClasses = null);
            highlighted = !oldStyles || oldStyles.length != line.styles.length || resetState != highlighted && (!resetState || !highlighted || resetState.bgClass != highlighted.bgClass || resetState.textClass != highlighted.textClass);
            for (resetState = 0; !highlighted && resetState < oldStyles.length; ++resetState) {
              highlighted = oldStyles[resetState] != line.styles[resetState];
            }
            highlighted && changedLines.push(context.line);
    ...
    function updateDisplayIfNeeded(cm, update) {
      var display = cm.display, doc$jscomp$0 = cm.doc;
      ...
      var end = doc$jscomp$0.first + doc$jscomp$0.size, from = Math.max(update.visible.from - cm.options.viewportMargin, doc$jscomp$0.first), to = Math.min(end, update.visible.to + cm.options.viewportMargin);
      display.viewFrom < from && 20 > from - display.viewFrom && (from = Math.max(doc$jscomp$0.first, display.viewFrom));
      display.viewTo > to && 20 > display.viewTo - to && (to = Math.min(end, display.viewTo));
      sawCollapsedSpans && (from = visualLineNo(cm.doc, from), to = visualLineEndNo(cm.doc, to));
      doc$jscomp$0 = from != display.viewFrom || to != display.viewTo || display.lastWrapHeight != update.wrapperHeight || display.lastWrapWidth != update.wrapperWidth;
      end = cm.display;
      0 == end.view.length || from >= end.viewTo || to <= end.viewFrom ? (end.view = buildViewArray(cm, from, to), end.viewFrom = from) : (end.viewFrom > from ? end.view = buildViewArray(cm, from, end.viewFrom).concat(end.view) : end.viewFrom < from && (end.view = end.view.slice(findViewIndex(cm, from))), end.viewFrom = from, end.viewTo < to ? end.view = end.view.concat(buildViewArray(cm, end.viewTo, to)) : end.viewTo > to && (end.view = end.view.slice(0, findViewIndex(cm, to))));
      end.viewTo = to;
      display.viewOffset = heightAtLine(getLine(cm.doc, display.viewFrom));
      cm.display.mover.style.top = display.viewOffset + "px";
      to = countDirtyView(cm);
      if (!doc$jscomp$0 && 0 == to && !update.force && display.renderedView == display.view && (null == display.updateLineNumbers || display.updateLineNumbers >= display.viewTo)) {
        return !1;
      }
      cm.hasFocus() ? from = null : (from = activeElt(rootNode(cm.display.wrapper))) && contains(cm.display.lineDiv, from) ? (from = {activeElt:from}, window.getSelection && (end = doc(cm).defaultView.getSelection(), end.anchorNode && end.extend && contains(cm.display.lineDiv, end.anchorNode) && (from.anchorNode = end.anchorNode, from.anchorOffset = end.anchorOffset, from.focusNode = end.focusNode, from.focusOffset = end.focusOffset))) : from = null;
      4 < to && (display.lineDiv.style.display = "none");
      patchDisplay(cm, display.updateLineNumbers, update.dims);
      4 < to && (display.lineDiv.style.display = "");
      display.renderedView = display.view;
      (to = from) && to.activeElt && to.activeElt != activeElt(rootNode(to.activeElt)) && (to.activeElt.focus(), !/^(INPUT|TEXTAREA)$/.test(to.activeElt.nodeName) && to.anchorNode && contains(document.body, to.anchorNode) && contains(document.body, to.focusNode) && (end = to.activeElt.ownerDocument, from = end.defaultView.getSelection(), end = end.createRange(), end.setEnd(to.anchorNode, to.anchorOffset), end.collapse(!1), from.removeAllRanges(), from.addRange(end), from.extend(to.focusNode, to.focusOffset)));
      removeChildren(display.cursorDiv);
      removeChildren(display.selectionDiv);
      display.gutters.style.height = display.sizer.style.minHeight = 0;
      doc$jscomp$0 && (display.lastWrapHeight = update.wrapperHeight, display.lastWrapWidth = update.wrapperWidth, startWorker(cm, 400));
    ...
    function postUpdateDisplay(cm, update) {
      for (var viewport = update.viewport, first = !0;; first = !1) {
        if (first && cm.options.lineWrapping && update.oldDisplayWidth != displayWidth(cm)) {
          first && (update.visible = visibleLines(cm.display, cm.doc, viewport));
        } else {
        ...
        first = measureForScrollbars(cm);
        updateSelection(cm);
        updateScrollbars(cm, first);
        setDocumentHeight(cm, first);
        update.force = !1;
      }
    ...
    function updateDisplaySimple(cm, viewport) {
      viewport = new DisplayUpdate(cm, viewport);
      if (updateDisplayIfNeeded(cm, viewport)) {
        updateHeightsInViewport(cm);
        postUpdateDisplay(cm, viewport);
    ...
    function patchDisplay(cm, updateNumbersFrom, dims) {
      ...
      var display = cm.display, lineNumbers = cm.options.lineNumbers, container = display.lineDiv, cur = container.firstChild, view = display.view;
      display = display.viewFrom;
      for (var i = 0; i < view.length; i++) {
        ...
        if (!lineView.hidden) {
          if (lineView.node && lineView.node.parentNode == container) {
            ...
            cur = lineNumbers && null != updateNumbersFrom && updateNumbersFrom <= display && lineView.lineNumber;
            lineView.changes && (-1 < indexOf(lineView.changes, "gutter") && (cur = !1), updateLineForChanges(cm, lineView, display, dims));
            cur && (removeChildren(lineView.lineNumber), lineView.lineNumber.appendChild(document.createTextNode(lineNumberFor(cm.options, display))));
            cur = lineView.node.nextSibling;
          } else {
            var node = buildLineElement(cm, lineView, display, dims);
            container.insertBefore(node, cur);
          }
        }
        display += lineView.size;
      }
      for (; cur;) {
        cur = rm(cur);
      }
    ...
    function maybeUpdateLineNumberWidth(cm) {
      ...
      var doc = cm.doc;
      doc = lineNumberFor(cm.options, doc.first + doc.size - 1);
      var display = cm.display;
      if (doc.length != display.lineNumChars) {
        var test = display.measure.appendChild(elt("div", [elt("div", doc)], "CodeMirror-linenumber CodeMirror-gutter-elt")), innerW = test.firstChild.offsetWidth;
        test = test.offsetWidth - innerW;
        display.lineGutter.style.width = "";
        display.lineNumInnerWidth = Math.max(innerW, display.lineGutter.offsetWidth - test) + 1;
        display.lineNumWidth = display.lineNumInnerWidth + test;
        display.lineNumChars = display.lineNumInnerWidth ? doc.length : -1;
    ...
    function renderGutters(display) {
      ...
      for (var i = 0; i < specs.length; ++i) {
        var ref = specs[i], className = ref.className;
        ref = ref.style;
        var gElt = gutters.appendChild(elt("div", null, "CodeMirror-gutter " + className));
        ref && (gElt.style.cssText = ref);
    ...
    function Display(place, doc, input, options) {
      ...
      this.heightForcer = elt("div", null, null, "position: absolute; height: 50px; width: 1px;");
      ...
      chrome && 105 <= chrome_version && (this.wrapper.style.clipPath = "inset(0px)");
    ...
    function wheelEventPixels(e) {
      e = wheelEventDelta(e);
      e.x *= wheelPixelsPerUnit;
      e.y *= wheelPixelsPerUnit;
      return e;
    }
    function onScrollWheel(cm, e) {
      ...
      var delta = wheelEventDelta(e), dx = delta.x;
      delta = delta.y;
      var pixelsPerUnit = wheelPixelsPerUnit;
      0 === e.deltaMode && (dx = e.deltaX, delta = e.deltaY, pixelsPerUnit = 1);
      ...
      if (dx && canScrollX || delta && canScrollY) {
        if (delta && mac && webkit) {
          ...
          a: for (; canScrollX != scroll; canScrollX = canScrollX.parentNode) {
            for (var i = 0; i < view.length; i++) {
              if (view[i].node == canScrollX) {
        ...
        !dx || gecko || presto || null == pixelsPerUnit ? (delta && null != pixelsPerUnit && (pixelsPerUnit *= delta, canScrollY = cm.doc.scrollTop, canScrollX = canScrollY + display.wrapper.clientHeight, 0 > pixelsPerUnit ? canScrollY = Math.max(0, canScrollY + pixelsPerUnit - 50) : canScrollX = Math.min(cm.doc.height, canScrollX + pixelsPerUnit + 50), updateDisplaySimple(cm, {top:canScrollY, bottom:canScrollX})), 20 > wheelSamples && 0 !== e.deltaMode && (null == display.wheelStartX ? (display.wheelStartX = 
        scroll.scrollLeft, display.wheelStartY = scroll.scrollTop, display.wheelDX = dx, display.wheelDY = delta, setTimeout(function() {
          if (null != display.wheelStartX) {
            var movedX = scroll.scrollLeft - display.wheelStartX, movedY = scroll.scrollTop - display.wheelStartY;
            movedX = movedY && display.wheelDY && movedY / display.wheelDY || movedX && display.wheelDX && movedX / display.wheelDX;
            display.wheelStartX = display.wheelStartY = null;
            movedX && (wheelPixelsPerUnit = (wheelPixelsPerUnit * wheelSamples + movedX) / (wheelSamples + 1), ++wheelSamples);
          }
        }, 200)) : (display.wheelDX += dx, display.wheelDY += delta))) : (delta && canScrollY && updateScrollTop(cm, Math.max(0, scroll.scrollTop + delta * pixelsPerUnit)), setScrollLeft(cm, Math.max(0, scroll.scrollLeft + dx * pixelsPerUnit)), (!delta || delta && canScrollY) && e_preventDefault(e), display.wheelStartX = null);
      }
    }
    function normalizeSelection(cm, ranges, primIndex) {
      cm = cm && cm.options.selectionsMayTouch;
      primIndex = ranges[primIndex];
      ranges.sort(function(a, b) {
        return cmp(a.from(), b.from());
      });
      primIndex = indexOf(ranges, primIndex);
      for (var i = 1; i < ranges.length; i++) {
        var cur = ranges[i], prev = ranges[i - 1], diff = cmp(prev.to(), cur.from());
        if (cm && !cur.empty() ? 0 < diff : 0 <= diff) {
          diff = minPos(prev.from(), cur.from());
          var to = maxPos(prev.to(), cur.to());
          cur = prev.empty() ? cur.from() == cur.head : prev.from() == prev.head;
          i <= primIndex && --primIndex;
          ranges.splice(--i, 2, new Range(cur ? to : diff, cur ? diff : to));
        }
      }
    ...
    function updateDoc(doc, change, markedSpans, estimateHeight) {
      function update(line, text, spans) {
        ...
        text = estimateHeight ? estimateHeight(line) : 1;
        text != line.height && updateLineHeight(line, text);
        signalLater(line, "change", line, change);
      }
      function linesFor(start, end) {
        for (var result = []; start < end; ++start) {
          result.push(new Line(text[start], markedSpans ? markedSpans[start] : null, estimateHeight));
        }
        return result;
      }
      var from = change.from, to = change.to, text = change.text, firstLine = getLine(doc, from.line), lastLine = getLine(doc, to.line), lastText = lst(text), lastSpans = markedSpans ? markedSpans[text.length - 1] : null, nlines = to.line - from.line;
      change.full ? (doc.insert(0, linesFor(0, text.length)), doc.remove(text.length, doc.size - text.length)) : isWholeLineUpdate(doc, change) ? (to = linesFor(0, text.length - 1), update(lastLine, lastLine.text, lastSpans), nlines && doc.remove(from.line, nlines), to.length && doc.insert(from.line, to)) : firstLine == lastLine ? 1 == text.length ? update(firstLine, firstLine.text.slice(0, from.ch) + lastText + firstLine.text.slice(to.ch), lastSpans) : (nlines = linesFor(1, text.length - 1), nlines.push(new Line(lastText + 
      firstLine.text.slice(to.ch), lastSpans, estimateHeight)), update(firstLine, firstLine.text.slice(0, from.ch) + text[0], markedSpans ? markedSpans[0] : null), doc.insert(from.line + 1, nlines)) : 1 == text.length ? (update(firstLine, firstLine.text.slice(0, from.ch) + text[0] + lastLine.text.slice(to.ch), markedSpans ? markedSpans[0] : null), doc.remove(from.line + 1, nlines)) : (update(firstLine, firstLine.text.slice(0, from.ch) + text[0], markedSpans ? markedSpans[0] : null), update(lastLine, 
      lastText + lastLine.text.slice(to.ch), lastSpans), lastSpans = linesFor(1, text.length - 1), 1 < nlines && doc.remove(from.line + 1, nlines - 1), doc.insert(from.line + 1, lastSpans));
    ...
    function addChangeToHistory(doc, change, selAfter, opId) {
      ...
      if (JSCompiler_temp = hist.lastOp == opId || hist.lastOrigin == change.origin && change.origin && ("+" == change.origin.charAt(0) && hist.lastModTime > time - (doc.cm ? doc.cm.options.historyEventDelay : 500) || "*" == change.origin.charAt(0))) {
        if (hist.lastOp == opId) {
          ...
          hist.done.length && !lst(hist.done).ranges ? cur = lst(hist.done) : 1 < hist.done.length && !hist.done[hist.done.length - 2].ranges ? (hist.done.pop(), cur = lst(hist.done)) : cur = void 0;
        }
        JSCompiler_temp = cur;
      }
      if (JSCompiler_temp) {
        ...
        for ((cur = lst(hist.done)) && cur.ranges || pushSelectionToHistory(doc.sel, hist.done), cur = {changes:[historyChangeFromChange(doc, change)], generation:hist.generation}, hist.done.push(cur); hist.done.length > hist.undoDepth;) {
    ...
    function mergeOldSpans(doc, change) {
      var old;
      if (old = change["spans_" + doc.id]) {
        for (var nw = [], i = 0; i < change.text.length; ++i) {
          var JSCompiler_temp_const = nw, JSCompiler_temp_const$jscomp$0 = JSCompiler_temp_const.push;
          var JSCompiler_inline_result = void 0;
          var spans = old[i];
          if (spans) {
            for (var i$jscomp$0 = 0; i$jscomp$0 < spans.length; ++i$jscomp$0) {
              spans[i$jscomp$0].marker.explicitlyCleared ? JSCompiler_inline_result || (JSCompiler_inline_result = spans.slice(0, i$jscomp$0)) : JSCompiler_inline_result && JSCompiler_inline_result.push(spans[i$jscomp$0]);
            }
            JSCompiler_inline_result = JSCompiler_inline_result ? JSCompiler_inline_result.length ? JSCompiler_inline_result : null : spans;
          } else {
      ...
      doc = stretchSpansOverChange(doc, change);
      if (!old) {
        return doc;
      }
      if (!doc) {
      ...
      for (change = 0; change < old.length; ++change) {
        if (nw = old[change], i = doc[change], nw && i) {
          a: for (JSCompiler_temp_const = 0; JSCompiler_temp_const < i.length; ++JSCompiler_temp_const) {
            JSCompiler_temp_const$jscomp$0 = i[JSCompiler_temp_const];
            for (JSCompiler_inline_result = 0; JSCompiler_inline_result < nw.length; ++JSCompiler_inline_result) {
              if (nw[JSCompiler_inline_result].marker == JSCompiler_temp_const$jscomp$0.marker) {
                continue a;
              }
            }
            nw.push(JSCompiler_temp_const$jscomp$0);
          }
        } else {
          i && (old[change] = i);
        }
    ...
    function extendRange(range, head, other, extend) {
      return extend ? (range = range.anchor, other && (extend = 0 > cmp(head, range), extend != 0 > cmp(other, range) ? (range = head, head = other) : extend != 0 > cmp(head, other) && (head = other)), new Range(range, head)) : new Range(other || head, head);
    }
    ...
    function extendSelections(doc, heads, options) {
      ...
      heads = normalizeSelection(doc.cm, out, doc.sel.primIndex);
      setSelection(doc, heads, options);
    }
    ...
    function filterSelectionChange(doc, sel, options) {
      options = {ranges:sel.ranges, update:function(ranges) {
        this.ranges = [];
        for (var i = 0; i < ranges.length; i++) {
          this.ranges[i] = new Range(clipPos(doc, ranges[i].anchor), clipPos(doc, ranges[i].head));
        }
      }, origin:options && options.origin};
      signal(doc, "beforeSelectionChange", doc, options);
      doc.cm && signal(doc.cm, "beforeSelectionChange", doc.cm, options);
      return options.ranges != sel.ranges ? normalizeSelection(doc.cm, options.ranges, options.ranges.length - 1) : sel;
    }
    ...
    function setSelection(doc, sel, options) {
      ...
      sel = doc.sel;
      var opId = doc.cm ? doc.cm.curOp.id : NaN, hist = doc.history, origin = options && options.origin, JSCompiler_temp;
      if (!(JSCompiler_temp = opId == hist.lastSelOp) && (JSCompiler_temp = origin && hist.lastSelOrigin == origin) && !(JSCompiler_temp = hist.lastModTime == hist.lastSelTime && hist.lastOrigin == origin)) {
        JSCompiler_temp = lst(hist.done);
        var ch = origin.charAt(0);
        JSCompiler_temp = "*" == ch || "+" == ch && JSCompiler_temp.ranges.length == sel.ranges.length && JSCompiler_temp.somethingSelected() == sel.somethingSelected() && new Date() - doc.history.lastSelTime <= (doc.cm ? doc.cm.options.historyEventDelay : 500);
      }
      JSCompiler_temp ? hist.done[hist.done.length - 1] = sel : pushSelectionToHistory(sel, hist.done);
    ...
    function skipAtomicInSelection(doc, sel, bias, mayClear) {
      for (var out, i = 0; i < sel.ranges.length; i++) {
        var range = sel.ranges[i], old = sel.ranges.length == doc.sel.ranges.length && doc.sel.ranges[i], newAnchor = skipAtomic(doc, range.anchor, old && old.anchor, bias, mayClear);
        old = range.head == range.anchor ? newAnchor : skipAtomic(doc, range.head, old && old.head, bias, mayClear);
        if (out || newAnchor != range.anchor || old != range.head) {
          out || (out = sel.ranges.slice(0, i)), out[i] = new Range(newAnchor, old);
        }
      }
          ...
          if ((null == sp.from || (preventCursorLeft ? sp.from <= pos.ch : sp.from < pos.ch)) && (null == sp.to || (preventCursorRight ? sp.to >= pos.ch : sp.to > pos.ch))) {
            ...
            if (m.atomic) {
              if (oldPos) {
                ...
                if (i && i.line == pos.line && (sp = cmp(i, oldPos)) && (0 > dir ? 0 > sp : 0 < sp)) {
                  return skipAtomicInner(doc, i, pos, dir, mayClear);
                }
              }
              oldPos = m.find(0 > dir ? -1 : 1);
              if (0 > dir ? preventCursorLeft : preventCursorRight) {
                oldPos = movePos(doc, oldPos, dir, oldPos.line == pos.line ? line : null);
              }
              return oldPos ? skipAtomicInner(doc, oldPos, pos, dir, mayClear) : null;
            }
          }
        }
      }
    ...
    function skipAtomic(doc, pos, oldPos, bias, mayClear) {
      bias = bias || 1;
      pos = skipAtomicInner(doc, pos, oldPos, bias, mayClear) || !mayClear && skipAtomicInner(doc, pos, oldPos, bias, !0) || skipAtomicInner(doc, pos, oldPos, -bias, mayClear) || !mayClear && skipAtomicInner(doc, pos, oldPos, -bias, !0);
      return pos ? pos : (doc.cantEdit = !0, Pos(doc.first, 0));
    }
    ...
    function makeChange(doc, change, ignoreReadOnly) {
      ...
      if (ignoreReadOnly = sawReadOnlySpans && !ignoreReadOnly && removeReadOnlyRanges(doc, change.from, change.to)) {
        for (var i = ignoreReadOnly.length - 1; 0 <= i; --i) {
          makeChangeInner(doc, {from:ignoreReadOnly[i].from, to:ignoreReadOnly[i].to, text:i ? [""] : change.text, origin:change.origin});
        }
      } else {
    ...
    function makeChangeFromHistory(doc, type, allowSelectionOnly) {
      ...
      if (!suppress || allowSelectionOnly) {
        for (var hist = doc.history, event, selAfter = doc.sel, source = "undo" == type ? hist.done : hist.undone, dest = "undo" == type ? hist.undone : hist.done, i = 0; i < source.length && (event = source[i], allowSelectionOnly ? !event.ranges || event.equals(doc.sel) : event.ranges); i++) {
        }
        if (i != source.length) {
          for (hist.lastOrigin = hist.lastSelOrigin = null;;) {
            if (event = source.pop(), event.ranges) {
              ...
              if (allowSelectionOnly && !event.equals(doc.sel)) {
                setSelection(doc, event, {clearRedo:!1});
                return;
              }
              selAfter = event;
            } else {
          ...
          allowSelectionOnly = function(i) {
            var change = event.changes[i];
            change.origin = type;
            if (filter && !filterChange(doc, change, !1)) {
          ...
          for (suppress = event.changes.length - 1; 0 <= suppress; --suppress) {
            if (hist = allowSelectionOnly(suppress)) {
              return hist.v;
            }
          }
        }
    ...
    function makeChangeSingleDoc(doc, change, selAfter, spans) {
      ...
      if (change.to.line < doc.first) {
        ...
        if (!(change.from.line > doc.lastLine())) {
          ...
          shift = doc.lastLine();
          change.to.line > shift && (change = {from:change.from, to:Pos(shift, getLine(doc, shift).text.length), text:[change.text[0]], origin:change.origin});
    ...
    function makeChangeSingleDocInEditor(cm, change, spans) {
      ...
      spans = hasHandler(cm, "changes");
      if ((doc = hasHandler(cm, "change")) || spans) {
        change = {from, to, text:change.text, removed:change.removed, origin:change.origin}, doc && signalLater(cm, "change", cm, change), spans && (cm.curOp.changeObjs || (cm.curOp.changeObjs = [])).push(change);
      }
      cm.display.selForContextMenu = null;
    }
    function replaceRange(doc, code, from, to, origin) {
      to || (to = from);
      0 > cmp(to, from) && (to = [to, from], from = to[0], to = to[1]);
    ...
    function rebaseHistArray(array, from, to, diff) {
      for (var i = 0; i < array.length; ++i) {
        ...
        if (sub.ranges) {
          for (sub.copied || (sub = array[i] = sub.deepCopy(), sub.copied = !0), ok = 0; ok < sub.ranges.length; ok++) {
            rebaseHistSelSingle(sub.ranges[ok].anchor, from, to, diff), rebaseHistSelSingle(sub.ranges[ok].head, from, to, diff);
          }
        } else {
    ...
    function rebaseHist(hist, change) {
      ...
      change = change.text.length - (to - from) - 1;
      rebaseHistArray(hist.done, from, to, change);
      rebaseHistArray(hist.undone, from, to, change);
    }
    ...
    function addLineWidget(doc, handle, node, options) {
      ...
      changeLine(doc, handle, "widget", function(line) {
        ...
        widget.line = line;
        cm && !lineIsHidden(doc, line) && (widgets = heightAtLine(line) < doc.scrollTop, updateLineHeight(line, line.height + widgetHeight(widget)), widgets && addToScrollTop(cm, widget.height), cm.curOp.forceUpdate = !0);
        return !0;
      });
    ...
    function markText(doc, from, to, options, type) {
      ...
      if (0 < type || 0 == type && !1 !== marker.clearWhenEmpty) {
        return marker;
      }
      marker.replacedWith && (marker.collapsed = !0, marker.widgetNode = eltP("span", [marker.replacedWith], "CodeMirror-widget"), options.handleMouseEvents || marker.widgetNode.setAttribute("cm-ignore-events", "true"), options.insertLeft && (marker.widgetNode.insertLeft = !0));
      if (marker.collapsed) {
      ...
      doc.iter(curLine, to.line + 1, function(line) {
        ...
        var span = new MarkedSpan(marker, curLine == from.line ? from.ch : null, curLine == to.line ? to.ch : null), op = doc.cm && doc.cm.curOp;
        (op = op && window.WeakSet && (op.markedSpans || (op.markedSpans = new WeakSet()))) && line.markedSpans && op.has(line.markedSpans) ? line.markedSpans.push(span) : (line.markedSpans = line.markedSpans ? line.markedSpans.concat([span]) : [span], op && op.add(line.markedSpans));
      ...
      if (cm) {
        ...
        if (marker.collapsed) {
          ...
          for (options = from.line; options <= to.line; options++) {
            regLineChange(cm, options, "text");
          }
        }
    ...
    function detachSharedMarkers(markers) {
      for (var loop = function(i) {
        i = markers[i];
        var linked = [i.primary.doc];
        linkedDocs(i.primary.doc, function(d) {
          return linked.push(d);
        });
        for (var j = 0; j < i.markers.length; j++) {
          var subMarker = i.markers[j];
          -1 == indexOf(linked, subMarker.doc) && (subMarker.parent = null, i.markers.splice(j--, 1));
        }
        ...
        if (pos && !cm.isReadOnly()) {
          if (files && files.length && window.FileReader && window.File) {
            ...
            if (cm.state.draggingText && -1 < cm.doc.sel.contains(pos)) {
              ...
              try {
                if (readTextFromFile = e.dataTransfer.getData("Text")) {
                  cm.state.draggingText && !cm.state.draggingText.copy && (i = cm.listSelections());
                  setSelectionNoUndo(cm.doc, simpleSelection(pos, pos));
                  if (i) {
                    for (files = 0; files < i.length; ++files) {
                      replaceRange(cm.doc, "", i[files].anchor, i[files].head, "drag");
                    }
                  }
                  cm.replaceSelection(readTextFromFile, "around", "paste");
    ...
    function lookupKey(key, map, handle, context) {
      ...
      if (map.fallthrough) {
        ...
        for (found = 0; found < map.fallthrough.length; found++) {
          var result = lookupKey(key, map.fallthrough[found], handle, context);
          if (result) {
            return result;
          }
        }
    ...
    function moveCharLogically(line, ch, dir) {
      ...
      return 0 > ch || ch > line.text.length ? null : ch;
    }
    function moveLogically(line, start, dir) {
      line = moveCharLogically(line, start.ch, dir);
      return null == line ? null : new Pos(start.line, line, 0 > dir ? "after" : "before");
    }
    function endOfLine(visually, cm, lineObj, lineNo, dir) {
      if (visually && ("rtl" == cm.doc.direction && (dir = -dir), visually = getOrder(lineObj, cm.doc.direction))) {
        visually = 0 > dir ? lst(visually) : visually[0];
        var sticky = 0 > dir == (1 == visually.level) ? "after" : "before";
        if (0 < visually.level || "rtl" == cm.doc.direction) {
          var prep = prepareMeasureForLine(cm, lineObj);
          var ch = 0 > dir ? lineObj.text.length - 1 : 0;
          var targetTop = measureCharPrepared(cm, prep, ch).top;
          ch = findFirst(function(ch) {
            return measureCharPrepared(cm, prep, ch).top == targetTop;
          }, 0 > dir == (1 == visually.level) ? visually.from : visually.to - 1, ch);
          "before" == sticky && (ch = moveCharLogically(lineObj, ch, 1));
        } else {
          ch = 0 > dir ? visually.to : visually.from;
        }
    ...
    function moveVisually(cm, line, start, dir) {
      ...
      if ("ltr" == cm.doc.direction && 0 == part.level % 2 && (0 < dir ? part.to > start.ch : part.from < start.ch)) {
        return moveLogically(line, start, dir);
      }
      var mv = function(pos, dir) {
        return moveCharLogically(line, pos instanceof Pos ? pos.ch : pos, dir);
      }, prep, getWrappedLineExtent = function(ch) {
      ...
      if ("rtl" == cm.doc.direction || 1 == part.level) {
        var moveInStorageOrder = 1 == part.level == 0 > dir, ch = mv(start, moveInStorageOrder ? 1 : -1);
        if (null != ch && (moveInStorageOrder ? ch <= part.to && ch <= wrappedLineExtent.end : ch >= part.from && ch >= wrappedLineExtent.begin)) {
          return new Pos(start.line, ch, moveInStorageOrder ? "before" : "after");
        }
      }
      part = function(partPos, dir, wrappedLineExtent) {
      ...
      if (partPos = part(partPos + dir, dir, wrappedLineExtent)) {
        return partPos;
      }
      wrappedLineExtent = 0 < dir ? wrappedLineExtent.end : mv(wrappedLineExtent.begin, -1);
      return null == wrappedLineExtent || 0 < dir && wrappedLineExtent == line.text.length || !(partPos = part(0 < dir ? 0 : bidi.length - 1, dir, getWrappedLineExtent(wrappedLineExtent))) ? null : partPos;
    }
    ...
    function lineStartSmart(cm, pos) {
      var start = lineStart(cm, pos.line), line = getLine(cm.doc, start.line);
      cm = getOrder(line, cm.doc.direction);
      return cm && 0 != cm[0].level ? start : (line = Math.max(start.ch, line.text.search(/\S/)), Pos(start.line, pos.line == start.line && pos.ch <= line && pos.ch ? 0 : line, start.sticky));
    }
    ...
    function dispatchKeyInner(cm, name, e, handle) {
      a: {
        ...
        handle = cm.options.extraKeys && lookupKey(name, cm.options.extraKeys, handle, cm) || lookupKey(name, cm.options.keyMap, handle, cm);
      }
      "multi" == handle && (cm.state.keySeq = name);
      "handled" == handle && signalLater(cm, "keyHandled", cm, name, e);
      if ("handled" == handle || "multi" == handle) {
    ...
    function onKeyDown(e) {
      if (!e.target || e.target == this.display.input.getField()) {
        if (this.curOp.focus = activeElt(rootNode(this.display.wrapper)), !signalDOMEvent(this, e)) {
      ...
      if (!(e.target && e.target != this.display.input.getField() || eventInWidget(this.display, e) || signalDOMEvent(this, e) || e.ctrlKey && !e.altKey || mac && e.metaKey)) {
        ...
        if (presto && keyCode == lastStoppedKey) {
          ...
          if (!presto || e.which && !(10 > e.which) || !handleKeyBinding(this, e)) {
            if (keyCode = String.fromCharCode(null == charCode ? keyCode : charCode), "\b" != keyCode && !handleCharBinding(this, e, keyCode)) {
    ...
    function leftButtonDown(cm, pos, repeat, event) {
      ie ? setTimeout(bind(ensureFocus, cm), 0) : cm.curOp.focus = activeElt(rootNode(cm.display.wrapper));
      var option = cm.getOption("configureMouse");
      option = option ? option(cm, repeat, event) : {};
      null == option.unit && (option.unit = (chromeOS ? event.shiftKey && event.metaKey : event.altKey) ? "rectangle" : "single" == repeat ? "char" : "double" == repeat ? "word" : "line");
      if (null == option.extend || cm.doc.extend) {
        option.extend = cm.doc.extend || event.shiftKey;
      }
      ...
      cm.options.dragDrop && dragAndDrop && !cm.isReadOnly() && "single" == repeat && -1 < (contained = sel.contains(pos)) && (0 > cmp((contained = sel.ranges[contained]).from(), pos) || 0 < pos.xRel) && (0 < cmp(contained.to(), pos) || 0 > pos.xRel) ? leftButtonStartDrag(cm, event, pos, option) : leftButtonSelect(cm, event, pos, option);
    }
    ...
    function rangeForUnit(cm, pos, unit) {
      ...
      cm = unit(cm, pos);
      return new Range(cm.from, cm.to);
    }
    function leftButtonSelect(cm, event, start, behavior) {
      function extendTo(pos) {
        if (0 != cmp(lastPos, pos)) {
          if (lastPos = pos, "rectangle" == behavior.unit) {
            var ranges = [], tabSize = cm.options.tabSize, startCol = countColumn(getLine(doc, start.line).text, start.ch, tabSize), posCol = countColumn(getLine(doc, pos.line).text, pos.ch, tabSize), left = Math.min(startCol, posCol);
            startCol = Math.max(startCol, posCol);
            posCol = Math.min(start.line, pos.line);
            for (var end = Math.min(cm.lastLine(), Math.max(start.line, pos.line)); posCol <= end; posCol++) {
              var text = getLine(doc, posCol).text, leftPos = findColumn(text, left, tabSize);
              left == startCol ? ranges.push(new Range(Pos(posCol, leftPos), Pos(posCol, leftPos))) : text.length > leftPos && ranges.push(new Range(Pos(posCol, leftPos), Pos(posCol, findColumn(text, startCol, tabSize))));
            }
            ranges.length || ranges.push(new Range(start, start));
            setSelection(doc, normalizeSelection(cm, startSel.ranges.slice(0, ourIndex).concat(ranges), ourIndex), {origin:"*mouse", scroll:!1});
            cm.scrollIntoView(pos);
          } else {
            ranges = ourRange, left = rangeForUnit(cm, pos, behavior.unit), pos = ranges.anchor, 0 < cmp(left.anchor, pos) ? (tabSize = left.head, pos = minPos(ranges.from(), left.anchor)) : (tabSize = left.anchor, pos = maxPos(ranges.to(), left.head)), ranges = startSel.ranges.slice(0), ranges[ourIndex] = bidiSimplify(cm, new Range(clipPos(doc, pos), tabSize)), setSelection(doc, normalizeSelection(cm, ranges, ourIndex), sel_mouse);
          }
        }
      }
      function extend(e) {
        ...
        if (cur) {
          if (0 != cmp(cur, lastPos)) {
            cm.curOp.focus = activeElt(rootNode(cm.display.wrapper));
            extendTo(cur);
      ...
      "rectangle" == behavior.unit ? (behavior.addNew || (ourRange = new Range(start, start)), start = posFromMouse(cm, event, !0, !0), ourIndex = -1) : (event = rangeForUnit(cm, start, behavior.unit), ourRange = behavior.extend ? extendRange(ourRange, event.anchor, event.head, behavior.extend) : event);
    ...
    function bidiSimplify(cm, range) {
      ...
      if (0 == boundary || boundary == anchorLine.length) {
        return range;
      }
      head.line != anchor.line ? cm = 0 < (head.line - anchor.line) * ("ltr" == cm.doc.direction ? 1 : -1) : (cm = getBidiPartAt(anchorLine, head.ch, head.sticky), index = cm - index || (head.ch - anchor.ch) * (1 == part.level ? -1 : 1), cm = cm == boundary - 1 || cm == boundary ? 0 > index : 0 < index);
      anchorLine = anchorLine[boundary + (cm ? -1 : 0)];
      anchorLine = (boundary = cm == (1 == anchorLine.level)) ? anchorLine.from : anchorLine.to;
      boundary = boundary ? "after" : "before";
      return anchor.ch == anchorLine && anchor.sticky == boundary ? range : new Range(new Pos(anchor.line, anchorLine, boundary), head);
    }
    function gutterEvent(cm, e, type, prevent) {
      ...
      prevent = cm.display;
      var lineBox = prevent.lineDiv.getBoundingClientRect();
      if (mY > lineBox.bottom || !hasHandler(cm, type)) {
        return e_defaultPrevented(e);
      }
      mY -= lineBox.top - prevent.viewOffset;
      for (lineBox = 0; lineBox < cm.display.gutterSpecs.length; ++lineBox) {
        var g = prevent.gutters.childNodes[lineBox];
        if (g && g.getBoundingClientRect().right >= mX) {
          return mX = lineAtHeight(cm.doc, mY), signal(cm, type, cm, mX, cm.display.gutterSpecs[lineBox].className, e), e_defaultPrevented(e);
        }
      }
    }
    ...
    function dragDropChanged(cm, value, old) {
      !value != !(old && old != Init) && (old = cm.display.dragFunctions, value = value ? on : off, value(cm.display.scroller, "dragstart", old.start), value(cm.display.scroller, "dragenter", old.enter), value(cm.display.scroller, "dragover", old.over), value(cm.display.scroller, "dragleave", old.leave), value(cm.display.scroller, "drop", old.drop));
    }
    ...
    function CodeMirror(place, options) {
      ...
      place = this.display = new Display(place, doc, input, options);
      place.wrapper.CodeMirror = this;
      themeChanged(this);
      ...
      options.autofocus && !mobile && place.input.focus();
      ...
      for (doc = 0; doc < initHooks.length; ++doc) {
        initHooks[doc](this);
      }
      endOperation(this);
      webkit && options.lineWrapping && "optimizelegibility" == getComputedStyle(place.lineDiv).textRendering && (place.lineDiv.style.textRendering = "auto");
    }
    function registerEventHandlers(cm) {
      ...
      function farAway(touch, other) {
        ...
        touch = other.top - touch.top;
        return 400 < dx * dx + touch * touch;
      }
      var d = cm.display;
      ...
      on(d.scroller, "touchstart", function(e) {
        var JSCompiler_temp;
        if (JSCompiler_temp = !signalDOMEvent(cm, e)) {
          1 != e.touches.length ? JSCompiler_temp = !1 : (JSCompiler_temp = e.touches[0], JSCompiler_temp = 1 >= JSCompiler_temp.radiusX && 1 >= JSCompiler_temp.radiusY), JSCompiler_temp = !JSCompiler_temp;
        }
        JSCompiler_temp && !gutterEvent(cm, e, "gutterClick", !0) && (d.input.ensurePolled(), clearTimeout(touchFinished), JSCompiler_temp = +new Date(), d.activeTouch = {start:JSCompiler_temp, moved:!1, prev:300 >= JSCompiler_temp - prevTouch.end ? prevTouch : null}, 1 == e.touches.length && (d.activeTouch.left = e.touches[0].pageX, d.activeTouch.top = e.touches[0].pageY));
      });
      ...
      on(d.scroller, "touchend", function(e) {
        ...
        if (touch && !eventInWidget(d, e) && null != touch.left && !touch.moved && 300 > new Date() - touch.start) {
          ...
          cm.setSelection(touch.anchor, touch.head);
          cm.focus();
          e_preventDefault(e);
        }
        finishTouch();
      ...
      d.dragFunctions = {enter:function(e) {
        ...
        if (!signalDOMEvent(cm, e)) {
          ...
          if (pos) {
            ...
            cm.display.dragCursor || (cm.display.dragCursor = elt("div", null, "CodeMirror-cursors CodeMirror-dragcursors"), cm.display.lineSpace.insertBefore(cm.display.dragCursor, cm.display.cursorDiv));
    ...
    function indentLine(cm, n, how, aggressive) {
      ...
      if (!aggressive && !/\S/.test(line.text)) {
        var indentation = 0;
        how = "not";
      } else if ("smart" == how && (indentation = doc.mode.indent(state, line.text.slice(curSpaceString.length), line.text), indentation == Pass || 150 < indentation)) {
        if (!aggressive) {
          return;
        }
        how = "prev";
      ...
      if (cm.options.indentWithTabs) {
        for (cm = Math.floor(indentation / tabSize); cm; --cm) {
          aggressive += tabSize, how += "\t";
        }
      }
      aggressive < indentation && (how += spaceStr(indentation - aggressive));
      if (how != curSpaceString) {
        return replaceRange(doc, how, Pos(n, 0), Pos(n, curSpaceString.length), "+input"), line.stateAfter = null, !0;
      }
      for (tabSize = 0; tabSize < doc.sel.ranges.length; tabSize++) {
        if (line = doc.sel.ranges[tabSize], line.head.line == n && line.head.ch < curSpaceString.length) {
          n = Pos(n, curSpaceString.length);
          replaceOneSelection(doc, tabSize, new Range(n, n));
          break;
        }
      }
    }
    function applyTextInput(cm, inserted, deleted, sel, origin) {
      ...
      if (paste && 1 < sel.ranges.length) {
        if (lastCopied && lastCopied.text.join("\n") == inserted) {
          ...
          textLines.length == sel.ranges.length && cm.options.pasteLinesPerSelection && (multiPaste = map(textLines, function(l) {
            return [l];
          }));
        }
      }
      ...
      for (var i$1 = sel.ranges.length - 1; 0 <= i$1; i$1--) {
        ...
        range.empty() && (deleted && 0 < deleted ? from = Pos(from.line, from.ch - deleted) : cm.state.overwrite && !paste ? to = Pos(to.line, Math.min(getLine(doc, to.line).text.length, to.ch + lst(textLines).length)) : paste && lastCopied && lastCopied.lineWise && lastCopied.text.join("\n") == textLines.join("\n") && (from = to = Pos(from.line, 0)));
        range = {from, to, text:multiPaste ? multiPaste[i$1 % multiPaste.length] : textLines, origin:origin || (paste ? "paste" : cm.state.cutIncoming > recent ? "cut" : "+input")};
        makeChange(cm.doc, range);
        signalLater(cm, "inputRead", cm, range);
      }
    ...
    function hiddenTextarea() {
      var te = elt("textarea", null, null, "position: absolute; bottom: -1em; padding: 0; width: 1px; height: 1em; min-height: 1em; outline: none"), div = elt("div", [te], null, "overflow: hidden; position: relative; width: 3px; height: 0px;");
    ...
    function findPosH(doc, pos, dir, unit, visually) {
      function moveOnce(boundToLine) {
        if ("codepoint" == unit) {
          var next = lineObj.text.charCodeAt(pos.ch + (0 < dir ? 0 : -1));
          next = isNaN(next) ? null : new Pos(pos.line, Math.max(0, Math.min(lineObj.text.length, pos.ch + dir * ((0 < dir ? 55296 <= next && 56320 > next : 56320 <= next && 57343 > next) ? 2 : 1))), -dir);
        } else {
          next = visually ? moveVisually(doc.cm, lineObj, pos, dir) : moveLogically(lineObj, pos, dir);
        }
        if (null == next) {
          if (boundToLine = !boundToLine) {
            boundToLine = pos.line + lineDir, boundToLine < doc.first || boundToLine >= doc.first + doc.size ? boundToLine = !1 : (pos = new Pos(boundToLine, pos.ch, pos.sticky), boundToLine = lineObj = getLine(doc, boundToLine));
          }
          if (boundToLine) {
      ...
      if ("char" == unit || "codepoint" == unit) {
        ...
        for (var sawType = null, group = "group" == unit, helper = doc.cm && doc.cm.getHelper(pos, "wordChars"), first = !0; !(0 > dir) || moveOnce(!first); first = !1) {
          ...
          !group || first || cur || (cur = "s");
          if (sawType && sawType != cur) {
      ...
      origDir = skipAtomic(doc, pos, oldPos, origDir, !0);
      equalCursorPos(oldPos, origDir) && (origDir.hitSide = !0);
      return origDir;
    }
    function findPosV(cm, pos, dir, unit) {
      var doc$jscomp$0 = cm.doc, x = pos.left;
      if ("page" == unit) {
        var y = Math.min(cm.display.wrapper.clientHeight, doc(cm).defaultView.innerHeight || doc$jscomp$0(cm).documentElement.clientHeight);
        y = Math.max(y - .5 * textHeight(cm.display), 3);
        y = (0 < dir ? pos.bottom : pos.top) + dir * y;
      } else {
        "line" == unit && (y = 0 < dir ? pos.bottom + 3 : pos.top - 3);
      }
      for (;;) {
        pos = coordsChar(cm, x, y);
        if (!pos.outside) {
          break;
        }
        if (0 > dir ? 0 >= y : y >= doc$jscomp$0.height) {
          pos.hitSide = !0;
          break;
        }
        y += 5 * dir;
    ...
    function posToDOM(cm, pos) {
      ...
      view = mapFromLineView(view, line, pos.line);
      cm = getOrder(line, cm.doc.direction);
      line = "left";
      cm && (line = getBidiPartAt(cm, pos.ch) % 2 ? "right" : "left");
      pos = nodeAndOffsetInLineMap(view.map, pos.ch, line);
      pos.offset = "right" == pos.collapse ? pos.end : pos.start;
      return pos;
    }
      ...
      function walk(node) {
        if (1 == node.nodeType) {
          ...
          if (cmText) {
            ...
            cmText = node.getAttribute("cm-marker");
            var range;
            if (cmText) {
              node = cm.findMarks(Pos(fromLine, 0), Pos(toLine + 1, 0), recognizeMarker(+cmText)), node.length && (range = node[0].find(0)) && addText(getBetween(cm.doc, range.from, range.to).join(lineSep));
            } else {
              if ("false" != node.getAttribute("contenteditable") && (range = /^(pre|div|p|li|table|br)$/i.test(node.nodeName), /^br$/i.test(node.nodeName) || 0 != node.textContent.length)) {
                range && close();
                for (cmText = 0; cmText < node.childNodes.length; cmText++) {
                  walk(node.childNodes[cmText]);
                }
          ...
          3 == node.nodeType && addText(node.nodeValue.replace(/\u200b/g, "").replace(/\u00a0/g, " "));
        }
      }
    ...
    function locateNodeInLineView(lineView, node, offset) {
        ...
        for (var i = -1; i < (maps ? maps.length : 0); i++) {
          for (var map = 0 > i ? measure.map : maps[i], j = 0; j < map.length; j += 3) {
            ...
            if (curNode == textNode || curNode == topNode) {
              topNode = lineNo(0 > i ? lineView.line : lineView.rest[i]);
              i = map[j] + offset;
              if (0 > offset || curNode != textNode) {
                i = map[j + (offset ? 1 : 0)];
              }
              return Pos(topNode, i);
            }
          }
        }
      }
      ...
      if (node == wrapper && (bad = !0, node = wrapper.childNodes[offset], offset = 0, !node)) {
        return offset = lineView.rest ? lst(lineView.rest) : lineView.line, badPos(Pos(lineNo(offset), offset.text.length), bad);
      }
      var textNode = 3 == node.nodeType ? node : null, topNode = node;
      textNode || 1 != node.childNodes.length || 3 != node.firstChild.nodeType || (textNode = node.firstChild, offset && (offset = textNode.nodeValue.length));
      for (; topNode.parentNode != wrapper;) {
        topNode = topNode.parentNode;
      }
      var measure = lineView.measure, maps = measure.maps;
      if (node = find(textNode, topNode, offset)) {
        return badPos(node, bad);
      }
      wrapper = topNode.nextSibling;
      for (textNode = textNode ? textNode.nodeValue.length - offset : 0; wrapper; wrapper = wrapper.nextSibling) {
        if (node = find(wrapper, wrapper.firstChild, 0)) {
          return badPos(Pos(node.line, node.ch - textNode), bad);
        }
        textNode += wrapper.textContent.length;
      }
      for (topNode = topNode.previousSibling; topNode; topNode = topNode.previousSibling) {
        if (node = find(topNode, topNode.firstChild, -1)) {
          return badPos(Pos(node.line, node.ch + offset), bad);
        }
        offset += topNode.textContent.length;
      }
    }
    ...
    var flipCtrlCmd = mac && (qtwebkit || presto && (null == presto_version || 12.11 > presto_version)), captureRightClick = gecko || ie && 9 <= ie_version, rmClass = function(node, cls) {
      var current = node.className;
      if (cls = classTest(cls).exec(current)) {
        var after = current.slice(cls.index + cls[0].length);
        node.className = current.slice(0, cls.index) + (after ? cls[1] + after : "");
      }
    };
    ...
    Delayed.prototype.set = function(ms, f) {
      this.f = f;
      f = +new Date() + ms;
      if (!this.id || f < this.time) {
        clearTimeout(this.id), this.id = setTimeout(this.handler, ms), this.time = f;
      }
    };
    ...
    bidiOther = null, bidiOrdering = function() {
      ...
      return function(str, direction) {
        ...
        for (var len = str.length, types = [], i = 0; i < len; ++i) {
          var JSCompiler_temp_const = types, JSCompiler_temp_const$jscomp$0 = JSCompiler_temp_const.push;
          var JSCompiler_inline_result = str.charCodeAt(i);
          JSCompiler_inline_result = 247 >= JSCompiler_inline_result ? "bbbbbbbbbtstwsbbbbbbbbbbbbbbssstwNN%%%NNNNNN,N,N1111111111NNNNNNNLLLLLLLLLLLLLLLLLLLLLLLLLLNNNNNNLLLLLLLLLLLLLLLLLLLLLLLLLLNNNNbbbbbbsbbbbbbbbbbbbbbbbbbbbbbbbbb,N%%%%NNNNLNNNNN%%11NLNNN1LNNNNNLLLLLLLLLLLLLLLLLLLLLLLNLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLN".charAt(JSCompiler_inline_result) : 1424 <= JSCompiler_inline_result && 1524 >= JSCompiler_inline_result ? "R" : 1536 <= JSCompiler_inline_result && 1785 >= JSCompiler_inline_result ? 
        ...
        i = 0;
        for (JSCompiler_temp_const = outerType; i < len; ++i) {
          JSCompiler_temp_const$jscomp$0 = types[i], "m" == JSCompiler_temp_const$jscomp$0 ? types[i] = JSCompiler_temp_const : JSCompiler_temp_const = JSCompiler_temp_const$jscomp$0;
        }
        i = 0;
        for (JSCompiler_temp_const = outerType; i < len; ++i) {
          JSCompiler_temp_const$jscomp$0 = types[i], "1" == JSCompiler_temp_const$jscomp$0 && "r" == JSCompiler_temp_const ? types[i] = "n" : isStrong.test(JSCompiler_temp_const$jscomp$0) && (JSCompiler_temp_const = JSCompiler_temp_const$jscomp$0, "r" == JSCompiler_temp_const$jscomp$0 && (types[i] = "R"));
        }
        i = 1;
        for (JSCompiler_temp_const = types[0]; i < len - 1; ++i) {
          JSCompiler_temp_const$jscomp$0 = types[i], "+" == JSCompiler_temp_const$jscomp$0 && "1" == JSCompiler_temp_const && "1" == types[i + 1] ? types[i] = "1" : "," != JSCompiler_temp_const$jscomp$0 || JSCompiler_temp_const != types[i + 1] || "1" != JSCompiler_temp_const && "n" != JSCompiler_temp_const || (types[i] = JSCompiler_temp_const), JSCompiler_temp_const = JSCompiler_temp_const$jscomp$0;
        }
        for (i = 0; i < len; ++i) {
          if (JSCompiler_temp_const = types[i], "," == JSCompiler_temp_const) {
            types[i] = "N";
          } else if ("%" == JSCompiler_temp_const) {
            for (JSCompiler_temp_const = i + 1; JSCompiler_temp_const < len && "%" == types[JSCompiler_temp_const]; ++JSCompiler_temp_const) {
            }
            for (JSCompiler_temp_const$jscomp$0 = i && "!" == types[i - 1] || JSCompiler_temp_const < len && "1" == types[JSCompiler_temp_const] ? "1" : "N"; i < JSCompiler_temp_const; ++i) {
              types[i] = JSCompiler_temp_const$jscomp$0;
            }
            i = JSCompiler_temp_const - 1;
          }
        }
        i = 0;
        for (JSCompiler_temp_const = outerType; i < len; ++i) {
          JSCompiler_temp_const$jscomp$0 = types[i], "L" == JSCompiler_temp_const && "1" == JSCompiler_temp_const$jscomp$0 ? types[i] = "L" : isStrong.test(JSCompiler_temp_const$jscomp$0) && (JSCompiler_temp_const = JSCompiler_temp_const$jscomp$0);
        }
        for (JSCompiler_temp_const = 0; JSCompiler_temp_const < len; ++JSCompiler_temp_const) {
          if (isNeutral.test(types[JSCompiler_temp_const])) {
            for (i = JSCompiler_temp_const + 1; i < len && isNeutral.test(types[i]); ++i) {
            }
            JSCompiler_temp_const$jscomp$0 = "L" == (JSCompiler_temp_const ? types[JSCompiler_temp_const - 1] : outerType);
            for (JSCompiler_temp_const$jscomp$0 = JSCompiler_temp_const$jscomp$0 == ("L" == (i < len ? types[i] : outerType)) ? JSCompiler_temp_const$jscomp$0 ? "L" : "R" : outerType; JSCompiler_temp_const < i; ++JSCompiler_temp_const) {
              types[JSCompiler_temp_const] = JSCompiler_temp_const$jscomp$0;
            }
            JSCompiler_temp_const = i - 1;
          }
        }
        outerType = [];
        var m;
        for (i = 0; i < len;) {
          if (countsAsLeft.test(types[i])) {
            JSCompiler_temp_const = i;
            for (++i; i < len && countsAsLeft.test(types[i]); ++i) {
            }
            outerType.push(new BidiSpan(0, JSCompiler_temp_const, i));
          } else {
            var pos = i;
            JSCompiler_temp_const = outerType.length;
            JSCompiler_temp_const$jscomp$0 = "rtl" == direction ? 1 : 0;
            for (++i; i < len && "L" != types[i]; ++i) {
            }
            for (JSCompiler_inline_result = pos; JSCompiler_inline_result < i;) {
              if (countsAsNum.test(types[JSCompiler_inline_result])) {
                pos < JSCompiler_inline_result && (outerType.splice(JSCompiler_temp_const, 0, new BidiSpan(1, pos, JSCompiler_inline_result)), JSCompiler_temp_const += JSCompiler_temp_const$jscomp$0);
                pos = JSCompiler_inline_result;
                for (++JSCompiler_inline_result; JSCompiler_inline_result < i && countsAsNum.test(types[JSCompiler_inline_result]); ++JSCompiler_inline_result) {
                }
                outerType.splice(JSCompiler_temp_const, 0, new BidiSpan(2, pos, JSCompiler_inline_result));
                JSCompiler_temp_const += JSCompiler_temp_const$jscomp$0;
                pos = JSCompiler_inline_result;
              } else {
                ++JSCompiler_inline_result;
              }
            }
            pos < i && outerType.splice(JSCompiler_temp_const, 0, new BidiSpan(1, pos, i));
          }
        }
        "ltr" == direction && (1 == outerType[0].level && (m = str.match(/^\s+/)) && (outerType[0].from = m[0].length, outerType.unshift(new BidiSpan(0, 0, m[0].length))), 1 == lst(outerType).level && (m = str.match(/\s+$/)) && (lst(outerType).to -= m[0].length, outerType.push(new BidiSpan(0, len - m[0].length, len))));
        return "rtl" == direction ? outerType.reverse() : outerType;
      };
    }(), noHandlers = [], on = function(emitter, type, f) {
      emitter.addEventListener ? emitter.addEventListener(type, f, !1) : emitter.attachEvent ? emitter.attachEvent("on" + type, f) : (emitter = emitter._handlers || (emitter._handlers = {}), emitter[type] = (emitter[type] || noHandlers).concat(f));
    ...
    StringStream.prototype.eat = function(match) {
      ...
      if ("string" == typeof match ? ch == match : ch && (match.test ? match.test(ch) : match(ch))) {
        return ++this.pos, ch;
      }
    };
    ...
    StringStream.prototype.match = function(pattern, consume, caseInsensitive) {
      if ("string" == typeof pattern) {
        ...
        if ((pattern = this.string.slice(this.pos).match(pattern)) && 0 < pattern.index) {
          return null;
        }
        pattern && !1 !== consume && (this.pos += pattern[0].length);
        return pattern;
      }
    };
    ...
    eventMixin(Line);
    var styleToClassCache = {}, styleToClassCacheWithMode = {}, operationGroup = null, orphanDelayedCallbacks = null, nullRect = {left:0, right:0, top:0, bottom:0}, measureText, NativeScrollbars = function(place, scroll, cm) {
      this.cm = cm;
    ...
    NativeScrollbars.prototype.enableZeroWidthBar = function(bar, delay, type) {
      function maybeDisable() {
        ...
        ("vert" == type ? document.elementFromPoint(box.right - 1, (box.top + box.bottom) / 2) : document.elementFromPoint((box.right + box.left) / 2, box.bottom - 1)) != bar ? bar.style.visibility = "hidden" : delay.set(1000, maybeDisable);
      }
    ...
    var scrollbarModel = {"native":NativeScrollbars, "null":NullScrollbars}, nextOpId = 0, DisplayUpdate = function(cm, viewport, force) {
      var display = cm.display;
    ...
    BranchChunk.prototype = {chunkSize:function() {
      ...
      25 > this.size - n && (1 < this.children.length || !(this.children[0] instanceof LeafChunk)) && (at = [], this.collapse(at), this.children = [new LeafChunk(at)], this.children[0].parent = this);
    }, collapse:function(lines) {
      for (var i = 0; i < this.children.length; ++i) {
      ...
      for (var i = 0; i < this.children.length; ++i) {
        ...
        if (at <= sz) {
          ...
          if (child.lines && 50 < child.lines.length) {
            for (lines = at = child.lines.length % 25 + 25; lines < child.lines.length;) {
              height = new LeafChunk(child.lines.slice(lines, lines += 25)), child.height -= height.height, this.children.splice(++i, 0, height), height.parent = this;
            }
      ...
      if (!(10 >= this.children.length)) {
        ...
        do {
          ...
          spilled = new BranchChunk(spilled);
          if (me.parent) {
            me.size -= spilled.size;
            me.height -= spilled.height;
            var myIndex = indexOf(me.parent.children, me);
            me.parent.children.splice(myIndex + 1, 0, spilled);
          } else {
            myIndex = new BranchChunk(me.children), myIndex.parent = me, me.children = [myIndex, spilled], me = myIndex;
          }
          spilled.parent = me.parent;
    ...
    TextMarker.prototype.clear = function() {
      if (!this.explicitlyCleared) {
        ...
        for (var max = found = null, i = 0; i < this.lines.length; ++i) {
          var line = this.lines[i], span = getMarkedSpanFor(line.markedSpans, this);
          cm && !this.collapsed ? regLineChange(cm, lineNo(line), "text") : cm && (null != span.to && (max = lineNo(line)), null != span.from && (found = lineNo(line)));
          for (var JSCompiler_temp_const = line, r = void 0, spans = line.markedSpans, span$jscomp$0 = span, i$jscomp$0 = 0; i$jscomp$0 < spans.length; ++i$jscomp$0) {
            spans[i$jscomp$0] != span$jscomp$0 && (r || (r = [])).push(spans[i$jscomp$0]);
          }
        ...
        if (cm && this.collapsed && !cm.options.lineWrapping) {
          for (i = 0; i < this.lines.length; ++i) {
            line = visualLine(this.lines[i]), span = lineLength(line), span > cm.display.maxLineLength && (cm.display.maxLine = line, cm.display.maxLineLength = span, cm.display.maxLineChanged = !0);
          }
        }
    ...
    TextMarker.prototype.changed = function() {
      ...
      pos && cm && runInOp(cm, function() {
        ...
        if (lineN = findViewForLine(cm, lineN)) {
          clearLineMeasurementCacheFor(lineN), cm.curOp.selectionChanged = cm.curOp.forceUpdate = !0;
        }
        cm.curOp.updateMaxLine = !0;
        lineIsHidden(widget.doc, line) || null == widget.height || (lineN = widget.height, widget.height = null, (lineN = widgetHeight(widget) - lineN) && updateLineHeight(line, line.height + lineN));
    ...
    TextMarker.prototype.detachLine = function(line) {
      ...
      !this.lines.length && this.doc.cm && (line = this.doc.cm.curOp, (line.maybeHiddenMarkers || (line.maybeHiddenMarkers = [])).push(this));
    };
    eventMixin(TextMarker);
    var SharedTextMarker = function(markers, primary) {
      this.markers = markers;
      this.primary = primary;
      for (primary = 0; primary < markers.length; ++primary) {
        markers[primary].parent = this;
      }
    };
    ...
    var nextDocId = 0, Doc = function(text, mode, firstLine, lineSep, direction) {
      ...
      firstLine = Pos(firstLine, 0);
      this.sel = simpleSelection(firstLine);
      ...
      updateDoc(this, {from:firstLine, to:firstLine, text});
    ...
    Doc.prototype = createObj(BranchChunk.prototype, {constructor:Doc, iter:function(from, to, op) {
      ...
      return !1 === lineSep ? from : "" === lineSep ? from.join("") : from.join(lineSep || this.lineSeparator());
    }, getLine:function(line) {
      return (line = this.getLineHandle(line)) && line.text;
      ...
      line = clipPos(this, "number" == typeof line ? Pos(line, ch || 0) : line);
      setSelection(this, simpleSelection(line, null), options);
    }), setSelection:docMethodOp(function(anchor$jscomp$0, head, options) {
      var anchor = clipPos(this, anchor$jscomp$0);
      anchor$jscomp$0 = clipPos(this, head || anchor$jscomp$0);
      setSelection(this, simpleSelection(anchor, anchor$jscomp$0), options);
    }), extendSelection:docMethodOp(function(head, other, options) {
      extendSelection(this, clipPos(this, head), other && clipPos(this, other), options);
    }), extendSelections:docMethodOp(function(heads, options) {
      extendSelections(this, clipPosArray(this, heads), options);
    }), extendSelectionsBy:docMethodOp(function(f, options) {
      f = map(this.sel.ranges, f);
      extendSelections(this, clipPosArray(this, f), options);
      ...
      if (code = collapse && "end" != collapse) {
        code = [];
        sel = origin = Pos(this.first, 0);
        for (i = 0; i < changes.length; i++) {
          var change = changes[i];
          range = offsetPos(change.from, origin, sel);
          var to = offsetPos(changeEnd(change), origin, sel);
          origin = change.to;
          sel = to;
          "around" == collapse ? (change = this.sel.ranges[i], change = 0 > cmp(change.head, change.anchor), code[i] = new Range(change ? to : range, change ? range : to)) : code[i] = new Range(range, range);
        }
        code = new Selection(code, this.sel.primIndex);
      }
      collapse = code;
      for (code = changes.length - 1; 0 <= code; code--) {
        makeChange(this, changes[code]);
      }
      ...
      for (i = 0; i < hist.undone.length; i++) {
      ...
      this.iter(function(line) {
        if (line = line.markedSpans) {
          for (var i = 0; i < line.length; ++i) {
            null != line[i].from && markers.push(line[i].marker);
          }
        }
      });
      return markers;
      ...
      from = new Doc(getLines(this, from, to), options.mode || this.modeOption, from, this.lineSep, this.direction);
      options.sharedHist && (from.history = this.history);
      (this.linked || (this.linked = [])).push({doc:from, sharedHist:options.sharedHist});
      from.linked = [{doc:this, isParent:!0, sharedHist:options.sharedHist}];
      options = findSharedMarkers(this);
      for (to = 0; to < options.length; to++) {
        var marker = options[to], pos = marker.find(), mFrom = from.clipPos(pos.from);
        pos = from.clipPos(pos.to);
        cmp(mFrom, pos) && (mFrom = markText(from, mFrom, pos, marker.primary, marker.primary.type), marker.markers.push(mFrom), mFrom.parent = marker);
      }
      return from;
    ...
    192:"`", 219:"[", 220:"\\", 221:"]", 222:"'", 224:"Mod", 63232:"Up", 63233:"Down", 63234:"Left", 63235:"Right", 63272:"Delete", 63273:"Home", 63275:"End", 63276:"PageUp", 63277:"PageDown", 63302:"Insert"}, i = 0; 10 > i; i++) {
    ...
    "Alt-U":"redoSelection", fallthrough:"basic"}, emacsy:{"Ctrl-F":"goCharRight", "Ctrl-B":"goCharLeft", "Ctrl-P":"goLineUp", "Ctrl-N":"goLineDown", "Ctrl-A":"goLineStart", "Ctrl-E":"goLineEnd", "Ctrl-V":"goPageDown", "Shift-Ctrl-V":"goPageUp", "Ctrl-D":"delCharAfter", "Ctrl-H":"delCharBefore", "Alt-Backspace":"delWordBefore", "Ctrl-K":"killLine", "Ctrl-T":"transposeChars", "Ctrl-O":"openLine"}, macDefault:{"Cmd-A":"selectAll", "Cmd-D":"deleteLine", "Cmd-Z":"undo", "Shift-Cmd-Z":"redo", "Cmd-Y":"redo", 
    ...
    "Cmd-Delete":"delWrappedLineRight", "Cmd-U":"undoSelection", "Shift-Cmd-U":"redoSelection", "Ctrl-Up":"goDocStart", "Ctrl-Down":"goDocEnd", fallthrough:["basic", "emacsy"]}};
    ...
    var commands = {selectAll, singleSelection:function(cm) {
      ...
      return deleteNearSelection(cm, function(range) {
        var top = cm.charCoords(range.head, "div").top + 5;
        top = cm.coordsChar({left:cm.display.lineDiv.offsetWidth + 100, top}, "div");
        return {from:range.from(), to:top};
      });
    }, undo:function(cm) {
      return cm.undo();
    }, redo:function(cm) {
      ...
      return cm.extendSelectionsBy(function(range) {
        range = range.head.line;
        var line = getLine(cm.doc, range);
        var visual = line;
        for (var merged; merged = collapsedSpanAtSide(visual, !1);) {
          visual = merged.find(1, !0).line;
        }
        visual != line && (range = lineNo(visual));
        return endOfLine(!0, cm, line, range, -1);
      }, {origin:"+move", bias:-1});
    }, goLineRight:function(cm) {
      return cm.extendSelectionsBy(function(range) {
        range = cm.cursorCoords(range.head, "div").top + 5;
        return cm.coordsChar({left:cm.display.lineDiv.offsetWidth + 100, top:range}, "div");
      }, sel_move);
    }, goLineLeft:function(cm) {
      return cm.extendSelectionsBy(function(range) {
        range = cm.cursorCoords(range.head, "div").top + 5;
        return cm.coordsChar({left:0, top:range}, "div");
      }, sel_move);
    }, goLineLeftSmart:function(cm) {
      return cm.extendSelectionsBy(function(range) {
        var top = cm.cursorCoords(range.head, "div").top + 5;
        top = cm.coordsChar({left:0, top}, "div");
      ...
      for (var spaces = [], ranges = cm.listSelections(), tabSize = cm.options.tabSize, i = 0; i < ranges.length; i++) {
        ...
        pos = countColumn(cm.getLine(pos.line), pos.ch, tabSize);
      ...
      return runInOp(cm, function() {
        for (var sels = cm.listSelections(), i = sels.length - 1; 0 <= i; i--) {
          cm.replaceRange(cm.doc.lineSeparator(), sels[i].anchor, sels[i].head, "+input");
        }
        sels = cm.listSelections();
        for (i = 0; i < sels.length; i++) {
          cm.indentLine(sels[i].from().line, null, !0);
        }
        ensureCursorVisible(cm);
      });
      ...
      function onCopyCut(e) {
        if (belongsToInput(e) && !signalDOMEvent(cm, e)) {
          if (cm.somethingSelected()) {
            lastCopied = {lineWise:!1, text:cm.getSelections()}, "cut" == e.type && cm.replaceSelection("", null, "cut");
    ...
    ContentEditableInput.prototype.showPrimarySelection = function() {
      var sel = this.getSelection(), cm = this.cm, prim = cm.doc.sel.primary(), from = prim.from();
      prim = prim.to();
      if (cm.display.viewTo == cm.display.viewFrom || from.line >= cm.display.viewTo || prim.line < cm.display.viewFrom) {
        sel.removeAllRanges();
      } else {
        ...
        if (!curAnchor || curAnchor.bad || !curFocus || curFocus.bad || 0 != cmp(minPos(curAnchor, curFocus), from) || 0 != cmp(maxPos(curAnchor, curFocus), prim)) {
          if (curAnchor = cm.display.view, from = from.line >= cm.display.viewFrom && posToDOM(cm, from) || {node:curAnchor[0].measure.map[2], offset:0}, prim = prim.line < cm.display.viewTo && posToDOM(cm, prim), prim || (prim = curAnchor[curAnchor.length - 1].measure, prim = prim.maps ? prim.maps[prim.maps.length - 1] : prim.map, prim = {node:prim[prim.length - 1], offset:prim[prim.length - 2] - prim[prim.length - 3]}), from && prim) {
            curAnchor = sel.rangeCount && sel.getRangeAt(0);
            try {
              var rng = range(from.node, from.offset, prim.offset, prim.node);
            } catch (e) {
            }
            rng && (!gecko && cm.state.focused ? (sel.collapse(from.node, from.offset), rng.collapsed || (sel.removeAllRanges(), sel.addRange(rng))) : (sel.removeAllRanges(), sel.addRange(rng)), curAnchor && null == sel.anchorNode ? sel.addRange(curAnchor) : gecko && this.startGracePeriod());
    ...
    ContentEditableInput.prototype.selectionInEditor = function() {
      ...
      sel = sel.getRangeAt(0).commonAncestorContainer;
      return contains(this.div, sel);
    };
    ContentEditableInput.prototype.focus = function() {
      "nocursor" != this.cm.options.readOnly && (this.selectionInEditor() && activeElt(rootNode(this.div)) == this.div || this.showSelection(this.prepareSelection(), !0), this.div.focus());
    };
    ...
    ContentEditableInput.prototype.pollContent = function() {
      ...
      var fromIndex;
      from.line == display.viewFrom || 0 == (fromIndex = findViewIndex(cm, from.line)) ? (sel = lineNo(display.view[0].line), fromIndex = display.view[0].node) : (sel = lineNo(display.view[fromIndex].line), fromIndex = display.view[fromIndex - 1].node.nextSibling);
      var toIndex = findViewIndex(cm, to.line);
      toIndex == display.view.length - 1 ? (to = display.viewTo - 1, display = display.lineDiv.lastChild) : (to = lineNo(display.view[toIndex + 1].line) - 1, display = display.view[toIndex + 1].node.previousSibling);
      if (!fromIndex) {
        return !1;
      }
      display = cm.doc.splitLines(domTextBetween(cm, fromIndex, display, sel, to));
      for (fromIndex = getBetween(cm.doc, Pos(sel, 0), Pos(to, getLine(cm.doc, to).text.length)); 1 < display.length && 1 < fromIndex.length;) {
        if (lst(display) == lst(fromIndex)) {
          display.pop(), fromIndex.pop(), to--;
        } else if (display[0] == fromIndex[0]) {
          display.shift(), fromIndex.shift(), sel++;
        } else {
          break;
        }
      }
      var cutFront = 0;
      toIndex = 0;
      for (var newTop = display[0], oldTop = fromIndex[0], maxCutFront = Math.min(newTop.length, oldTop.length); cutFront < maxCutFront && newTop.charCodeAt(cutFront) == oldTop.charCodeAt(cutFront);) {
        ++cutFront;
      }
      newTop = lst(display);
      oldTop = lst(fromIndex);
      for (maxCutFront = Math.min(newTop.length - (1 == display.length ? cutFront : 0), oldTop.length - (1 == fromIndex.length ? cutFront : 0)); toIndex < maxCutFront && newTop.charCodeAt(newTop.length - toIndex - 1) == oldTop.charCodeAt(oldTop.length - toIndex - 1);) {
        ++toIndex;
      }
      if (1 == display.length && 1 == fromIndex.length && sel == from.line) {
        for (; cutFront && cutFront > from.ch && newTop.charCodeAt(newTop.length - toIndex - 1) == oldTop.charCodeAt(oldTop.length - toIndex - 1);) {
          cutFront--, toIndex++;
        }
      }
      display[display.length - 1] = newTop.slice(0, newTop.length - toIndex).replace(/^\u200b+/, "");
      display[0] = display[0].slice(cutFront).replace(/\u200b+$/, "");
      from = Pos(sel, cutFront);
      sel = Pos(to, fromIndex.length ? lst(fromIndex).length - toIndex : 0);
      if (1 < display.length || display[0] || cmp(from, sel)) {
        return replaceRange(cm.doc, display, from, sel, "+input"), !0;
      }
    };
    ...
    TextareaInput.prototype.createField = function(_display) {
      ...
      _display = this.cm.options;
      disableBrowserMagic(this.textarea, _display.spellcheck, _display.autocorrect, _display.autocapitalize);
    };
    ...
    TextareaInput.prototype.prepareSelection = function() {
      ...
      if (cm.options.moveInputWithCursor) {
        cm = cursorCoords(cm, doc.sel.primary().head, "div");
    ...
    TextareaInput.prototype.reset = function(typing) {
      if (!(this.contextMenuPending || this.composing && typing)) {
        var cm = this.cm;
        this.resetting = !0;
        cm.somethingSelected() ? (this.prevInput = "", typing = cm.getSelection(), this.textarea.value = typing, cm.state.focused && selectInput(this.textarea), ie && 9 <= ie_version && (this.hasSelection = typing)) : typing || (this.prevInput = this.textarea.value = "", ie && 9 <= ie_version && (this.hasSelection = null));
        this.resetting = !1;
      }
    };
    ...
    TextareaInput.prototype.focus = function() {
      if ("nocursor" != this.cm.options.readOnly && (!mobile || activeElt(rootNode(this.textarea)) != this.textarea)) {
        try {
          this.textarea.focus();
        } catch (e) {
        }
    ...
    TextareaInput.prototype.poll = function() {
      ...
      var same = 0;
      for (first = Math.min(prevInput.length, text.length); same < first && prevInput.charCodeAt(same) == text.charCodeAt(same);) {
        ++same;
      }
      runInOp(cm, function() {
        applyTextInput(cm, text.slice(same), prevInput.length - same, null, this$1.composing ? "*compose" : null);
        1000 < text.length || -1 < text.indexOf("\n") ? input.value = this$1.prevInput = "" : this$1.prevInput = text;
    ...
    TextareaInput.prototype.onContextMenu = function(e) {
      ...
      if (pos && !presto) {
        ...
        if (webkit) {
          var oldScrollY = te.ownerDocument.defaultView.scrollY;
        }
        display.input.focus();
        webkit && te.ownerDocument.defaultView.scrollTo(null, oldScrollY);
        display.input.reset();
    ...
    (function(CodeMirror) {
      ...
      option("keyMap", "default", function(cm, val, old) {
        val = getKeyMap(val);
        (old = old != Init && getKeyMap(old)) && old.detach && old.detach(cm, val);
        val.attach && val.attach(cm, old || null);
      });
      ...
      CodeMirror.prototype = {constructor:CodeMirror, focus:function() {
        ...
        for (var ranges = this.doc.sel.ranges, end = -1, i = 0; i < ranges.length; i++) {
          ...
          if (range.empty()) {
            ...
            end = Math.min(this.lastLine(), range.line - (range.ch ? 0 : 1)) + 1;
            for (range = start; range < end; ++range) {
              indentLine(this, range, how);
            }
            range = this.doc.sel.ranges;
            0 == from.ch && ranges.length == range.length && 0 < range[i].from().ch && replaceOneSelection(this.doc, i, new Range(from, range[i].to()), sel_dontScroll);
          }
        }
        ...
        var styles = getLineStyles(this, getLine(this.doc, pos.line)), before = 0, after = (styles.length - 1) / 2;
        pos = pos.ch;
        if (0 == pos) {
          styles = styles[2];
        } else {
          for (;;) {
            ...
            if ((mid ? styles[2 * mid - 1] : 0) >= pos) {
              after = mid;
            } else if (styles[2 * mid + 1] < pos) {
              before = mid + 1;
            } else {
              styles = styles[2 * mid + 2];
              break;
            }
          }
        }
        before = styles ? styles.indexOf("overlay ") : -1;
        return 0 > before ? styles : 0 == before ? null : styles.slice(0, before - 1);
        ...
        pos = this.getModeAt(pos);
        if ("string" == typeof pos[type]) {
          help[pos[type]] && found.push(help[pos[type]]);
        } else if (pos[type]) {
          for (var i = 0; i < pos[type].length; i++) {
            var val = help[pos[type][i]];
            val && found.push(val);
          }
        } else {
          pos.helperType && help[pos.helperType] ? found.push(help[pos.helperType]) : help[pos.name] && found.push(help[pos.name]);
        }
        for (type = 0; type < help._global.length; type++) {
          i = help._global[type], i.pred(pos, this) && -1 == indexOf(found, i.val) && found.push(i.val);
        }
        return found;
        ...
        start = null == start ? range.head : "object" == typeof start ? clipPos(this.doc, start) : start ? range.from() : range.to();
        ...
        return intoCoordSystem(this, line, {top:0, left:0}, mode || "page", includeWidgets || end).top + (end ? this.doc.height - heightAtLine(line) : 0);
        ...
        scroll && (pos = calculateScrollPos(this, {left, top, right:left + node.offsetWidth, bottom:top + node.offsetHeight}), null != pos.scrollTop && updateScrollTop(this, pos.scrollTop), null != pos.scrollLeft && setScrollLeft(this, pos.scrollLeft));
      }, triggerOnKeyDown:methodOp(onKeyDown), triggerOnKeyPress:methodOp(onKeyPress), triggerOnKeyUp:onKeyUp, triggerOnMouseDown:methodOp(onMouseDown), execCommand:function(cmd) {
        ...
        from = clipPos(this.doc, from);
        for (var i = 0; i < amount && (from = findPosH(this.doc, from, dir, unit, visually), !from.hitSide); ++i) {
        }
        return from;
        ...
        for (from = 0; from < amount && (cur = cursorCoords(this, cur, "div"), null == goalColumn ? goalColumn = cur.left : cur.left = goalColumn, cur = findPosV(this, cur, dir, unit), !cur.hitSide); ++from) {
        }
        return cur;
        ...
        if (line) {
          ...
          for (startChar = isWordChar(startChar, helper) ? function(ch) {
            return isWordChar(ch, helper);
          } : /\s/.test(startChar) ? function(ch) {
            return /\s/.test(ch);
          } : function(ch) {
            return !/\s/.test(ch) && !isWordChar(ch);
          }; 0 < start && startChar(line.charAt(start - 1));) {
            --start;
          }
          for (; end < line.length && startChar(line.charAt(end));) {
            ++end;
          }
        }
        ...
        return this.display.input.getField() == activeElt(rootNode(this.display.wrapper));
      }, isReadOnly:function() {
        ...
        null == range ? (range = {from:this.doc.sel.primary().head, to:null}, null == margin && (margin = this.options.cursorScrollMargin)) : "number" == typeof range ? range = {from:Pos(range, 0), to:null} : null == range.from && (range = {from:range, to:null});
        range.to || (range.to = range.from);
        range.margin = margin || 0;
        null != range.from.line ? (resolveScrollToPos(this), this.curOp.scrollToPos = range) : scrollToCoordsRange(this, range.from, range.to, range.margin);
    ...
    var dontDelegate = "iter insert remove copy getEditor constructor".split(" "), prop;
    for (prop in Doc.prototype) {
      Doc.prototype.hasOwnProperty(prop) && 0 > indexOf(dontDelegate, prop) && (CodeMirror.prototype[prop] = function(method) {
        return function() {
