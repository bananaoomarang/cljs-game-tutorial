shadow$provide.module$node_modules$codemirror$mode$clojure$clojure = function(global, require, module, exports) {
  (function(mod) {
    ...
    CodeMirror.defineMode("clojure", function(options) {
      function base(stream, state) {
        ...
        if (stream.eat(/^;/)) {
          return stream.skipToEnd(), ["space", "comment"];
        }
        if (stream.eat(/^[#'@^`~]/)) {
          return [null, "meta"];
        }
        ...
        return matches ? "comment" === matches && "(" === state.lastToken ? (state.tokenize = inComment)(stream, state) : is(matches, atom) || ":" === matches.charAt(0) ? ["symbol", "atom"] : is(matches, specialForm) || is(matches, coreSymbol) ? ["symbol", "keyword"] : "(" === state.lastToken ? ["symbol", "builtin"] : ["symbol", "variable"] : (stream.next(), stream.eatWhile(function(c) {
          return !is(c, delimiter);
        }), [null, "error"]);
      }
      function inString(stream, state) {
        for (var escaped = !1, next; next = stream.next();) {
          if ('"' === next && !escaped) {
            state.tokenize = base;
            break;
          }
      ...
      function inComment(stream, state) {
        for (var parenthesisCount = 1, next; next = stream.next();) {
          if (")" === next && parenthesisCount--, "(" === next && parenthesisCount++, 0 === parenthesisCount) {
            stream.backUp(1);
            state.tokenize = base;
            break;
          }
      ...
      return {startState:function() {
        return {ctx:{prev:null, start:0, indentTo:0}, lastToken:null, tokenize:base};
      }, token:function(stream, state) {
        stream.sol() && "number" !== typeof state.ctx.indentTo && (state.ctx.indentTo = state.ctx.start + 1);
        var typeStylePair = state.tokenize(stream, state), type = typeStylePair[0];
        typeStylePair = typeStylePair[1];
        var current = stream.current();
        "space" !== type && ("(" === state.lastToken && null === state.ctx.indentTo ? "symbol" === type && is(current, hasBodyParameter) ? state.ctx.indentTo = state.ctx.start + options.indentUnit : state.ctx.indentTo = "next" : "next" === state.ctx.indentTo && (state.ctx.indentTo = stream.column()), state.lastToken = current);
        "open" === type ? state.ctx = {prev:state.ctx, start:stream.column(), indentTo:null} : "close" === type && (state.ctx = state.ctx.prev || state.ctx);
        return typeStylePair;
